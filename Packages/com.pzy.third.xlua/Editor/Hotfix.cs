...
namespace XLua
{
    public static class HotfixConfig
    {
        ...
        {
            var projectPath = Assembly.Load("Assembly-CSharp-firstpass").ManifestModule.FullyQualifiedName;
            Regex rgx = new Regex(@"^(.*)[\\/]Library[\\/]ScriptAssemblies[\\/]Assembly-CSharp-firstpass.dll$");
...
namespace XLua
{
    ...
    public static class Hotfix
    {
        ...
        // pzy: 注入程序的工作方式是动态遍历编辑器内存中的程序集，
        // 发现使用了 [Hotfix] 属性的程序集就进行注入，
        // 但是在安卓就热更方式下，目标平台不编译 Assembly-CSharp.dll 程序集
        // 注入程序会因为找不到文件而报错
        // 因此这一步需要项目的编译代码来决定是否执行
        //[PostProcessScene]
        //public static void PostProcessScene()
        //{
        //    HotfixInject("./Library/ScriptAssemblies", true);
        //}
        ...
        public static void HotfixInject()
        {
            HotfixInject("./Library/ScriptAssemblies", false);
        }

        public static void HotfixInject(string assemblyDir, bool mustSuccess = false)
        {
            ...
            //var id_map_file_path = CSObjectWrapEditor.GeneratorConfig.common_path + "$hotfix_id_map/hotfix_id_map.lua.txt";
            ...
            foreach (var injectAssemblyPath in injectAssemblyPaths)
            {
                ...
                // pzy: 中间程序集是在编辑器平台生成的，注入的目标程序集是在编译目标平台生成的
                // 其中的差异可能因此处理失败
                // 但是返回码并不会报告错误，因此需要对输出进行检查
                var output = hotfix_injection.StandardOutput.ReadToEnd();
                UnityEngine.Debug.Log(output);
                if(mustSuccess)
                {
                    if (output.Contains("try re-genertate code."))
                    {
                        throw new Exception("[Hotfix] injection fail");
                    }
                }
                ...
                // pzy: 检查是否有异常
                var exitCode = hotfix_injection.ExitCode;
                if(exitCode != 0)
                {
                    if(mustSuccess)
                    {
