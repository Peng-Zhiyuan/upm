    ...
    public static class TypeExtensions
    {
        ...
        public static string GetFriendlyName(this Type type)
        {
            ...
            else if (type.IsGenericType)
            {
                ...
                {
                    // pzy:
                    // 不知道为什么，System.EventHandler`1[TEventArgs] 的 fullName 返回空
                    // 应该要返回 System.EventHandler`1
                    // 手动拼凑一个

                    var @namespace = type.Namespace;
                    var name = type.Name;
                    fullName = $"{@namespace}.{name}";
                    UnityEngine.Debug.LogWarning(type + ", fullName: " + fullName + ", name: " + name + ", @namespace: " + @namespace);
                }
                ...
                return first + "<" + string.Join(", ", genericArgumentNames) + ">";
            }
            else
