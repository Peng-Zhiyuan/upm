...
namespace Loxodon.Framework.Examples.Bundle
{
    public abstract class AbstractDownloader : IDownloader
    {
        ...
        {
            var result = new ProgressResult<Progress, BundleManifest>();
        ...
        {
            var result = new ProgressResult<float, List<BundleInfo>>();
        ...
        {
            var downloads = new List<BundleInfo>();
            var bundleInfoList = manifest.GetAll();
            float last = Time.realtimeSinceStartup;
            int length = bundleInfoList.Length;

            for (int i = 0; i < bundleInfoList.Length; i++)
            {
