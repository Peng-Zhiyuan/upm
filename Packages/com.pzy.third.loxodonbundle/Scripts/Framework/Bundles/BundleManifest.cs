...
namespace Loxodon.Framework.Bundles
{
    ...
    public class BundleManifest : ISerializationCallbackReceiver
    {
        ...
        {
            //Debug.Log("[Luncher BundleManifest]: json: " + json);
            var ret = JsonUtility.FromJson<BundleManifest>(json);
            //Debug.Log("ret: " + ret);
            return ret;
        }
        ...
        {
            add { lock (_lock) { this.activeVariantsChanged += value; } }
            remove { lock (_lock) { this.activeVariantsChanged -= value; } }
        }
        ...
        /// <summary>
        /// Gets all BundleInfos.
        /// </summary>
        /// <returns></returns>
        public virtual List<BundleInfo> GetAllBundleInfo()
        {
            return new List<BundleInfo>(bundles.Values);
        }
            ...
            for (int i = 0; i < dependencies.Length; i++)
            {
                ...
                if (dependency.Equals(root))
                {
                    if (log.IsWarnEnabled)
                        log.WarnFormat("It has an loop reference between '{0}' and '{1}',it is recommended to redistribute their assets.", root.Name, info.Name);
