...
namespace Loxodon.Framework.Bundles
{
    public class DefaultBundle : IBundle
    {
        ...
        public virtual string[] GetBundleAssetNames(){
        ...
        public static bool SimulateLowIO;

        protected virtual IEnumerator DoLoadBundleAndDependencies(IProgressPromise<float, IBundle> promise)
        {
            Debug.Log("<color=#00FF00>[DefaultBundle] async Load native bundle: " + this.bundleInfo.Name + "</color>");
            if(Application.isEditor)
            {
                if(SimulateLowIO)
                {
                    var sleepTime = UnityEngine.Random.Range(0.5f, 2.5f);
        ...
        private Dictionary<string, bool> loadedNames = new Dictionary<string, bool>();
        public virtual Object LoadAsset(string name, System.Type type)
        {
            ...
            if(Application.isEditor)
            {
                if(!loadedNames.ContainsKey(fullName))
                {
                    Debug.Log("<color=#008800>[DefaultBundle] sync load native asset: " + name + "</color>");
                    loadedNames[fullName] = true;
                    Thread.Sleep(100);
                }
            }
        ...
        public virtual T LoadAsset<T>(string name) where T : Object
        {
            ...
            if(Application.isEditor)
            {
                if(!loadedNames.ContainsKey(fullName))
                {
                    Debug.Log("<color=#008888>[DefaultBundle] sync load native asset: " + name + "</color>");
                    loadedNames[fullName] = true;
                    Thread.Sleep(100);
                }
            }
        ...
        protected virtual IEnumerator DoLoadAssetAsync(IProgressPromise<float, Object> promise, string name, System.Type type)
        {
            ...
            if (!this.requestCache.TryGetValue(key, out request))
            {
                if(Application.isEditor)
                {
                    Debug.Log("<color=#008800>[DefaultBundle] async load native asset: " + name + "</color>");
                    if(SimulateLowIO)
                    {
                        var sleepTime = Random.Range(0.5f, 2.5f);
                        yield return new WaitForSeconds(sleepTime);
                    }
                }
        ...
        protected virtual IEnumerator DoLoadAssetAsync<T>(IProgressPromise<float, T> promise, string name) where T : Object
        {
            ...
            if (!this.requestCache.TryGetValue(key, out request))
            {
                ...
                if (Application.isEditor)
                {
                    Debug.Log("<color=#008800>[DefaultBundle] background load native asset: " + name + "</color>");
                    if (SimulateLowIO)
                    {
                        var sleepTime = Random.Range(0.5f, 2.5f);
                        yield return new WaitForSeconds(sleepTime);
