...
#pragma warning disable 0414, 0219
    public abstract class AbstractResources : IResources, IDisposable
    {
        ...
        public virtual string[] GetBundleAssetNames(string bundleName){
            UnityEngine.Debug.Log("[AbstractResource] GetBundleNames");
            // return GetBundle(bundleName).GetBundleAssetNames();
            return bundleManager.GetBundleAssetNames(bundleName);
        }
        
        public virtual string[] GetBundleNames(){
            UnityEngine.Debug.Log("[AbstractResources] GetBundleNames");
            return bundleManager.GetBundleNames();
        }

        public List<string> GetAllBundleNames()
        {
            var ret = bundleManager.GetAllBundleNames();
            return ret;
        }
        ...
        {
            //Debug.Log(this.bundleManager.GetType().Name);
            var ret =  this.bundleManager.LoadBundle(bundleNames, priority);
            return ret;
        }
        ...
        public AssetPathInfo ParsePath(string path)
        {
        ...
        // assetPath
        // like:
        //  bundles/_atlas/fight.prefab
        public virtual IProgressResult<float, T> LoadAssetAsync<T>(string assetPath) where T : Object
        {
            try
            {
                ...
                AssetPathInfo pathInfo = this.pathInfoParser.Parse(assetPath);
                if (pathInfo == null)
                    throw new System.Exception(string.Format("Not found the AssetBundle or parses the path info '{0}' failure.", assetPath));

                // 弱引用缓存，弱引用不打开，因此这里永远不会有缓存
                T asset = this.GetCache<T>(assetPath);
                if (asset != null)
                {
                    result.UpdateProgress(1f);
                    result.SetResult(asset);
                ...
                bundleResult.Callbackable().OnCallback((r) =>
                {
                    ...
                    using (IBundle bundle = r.Result)
                    {
                        ...
                        assetResult.Callbackable().OnCallback((ar) =>
                        {
                            ...
                            {
                                result.SetResult(ar.Result);
                                this.AddCache<T>(assetPath, ar.Result);
                            }
                        });
                    }
                });
        ...
        protected virtual IEnumerator DoLoadAssetsToMapAsync(IProgressPromise<float, Dictionary<string, Object>> promise, System.Type type, params string[] paths)
        {
            ...
            Dictionary<string, IProgressResult<float, Dictionary<string, Object>>> assetResults = new Dictionary<string, IProgressResult<float, Dictionary<string, Object>>>();
            IBundle[] bundles = bundleResult.Result;
            for (int i = 0; i < bundles.Length; i++)
            {
                using (IBundle bundle = bundles[i])
                {
                    ...
                    IProgressResult<float, Dictionary<string, Object>> assetResult = bundle.LoadAssetsToMapAsync(type, assetNames.ToArray());
                    assetResult.Callbackable().OnCallback(ar =>
                    {
                        if (ar.Exception != null)
                            return;
        ...
        public virtual IProgressResult<float, Dictionary<string, T>> LoadAssetsToMapAsync<T>(params string[] paths) where T : Object
        {
            try
            {
                if (paths == null || paths.Length <= 0)
                ...
                return Executors.RunOnCoroutine<float, Dictionary<string, T>>((promise) => DoLoadAssetsToMapAsync(promise, paths));
            }
            catch (System.Exception e)
            {
