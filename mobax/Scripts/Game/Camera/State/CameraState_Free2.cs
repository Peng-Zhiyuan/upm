...
public class CameraState_Free2 : BaseCameraState
{
    protected override CameraState GetState()
    {
        return CameraState.Free2;
    }
    ...
    public override void LateUpdate(float param_deltaTime)
    {
        ...
        if(fDisTarget != 0)
        Proxy.Distance = Mathf.Lerp(Proxy.Distance, fDisTarget, param_deltaTime * speed);
    ...
    private void UpdateCamera(float param_deltaTime)
    {
        ...
        //Proxy.Distance = dis;
        var dis = Proxy.Distance;

        var fov =
            (fDisMax - dis) * (MaxFov - MinFov) / (fDisMax - fDisMin) + MinFov;
        Proxy.FieldOfView = Mathf.Clamp(fov, MinFov, MaxFov);
    ...
    public float hMax = 25f;
    public float hMin = 10f;
    public float durT = 5f;
    public float fResetTime = 0;
    public float fDisMax = 5.5f;
    public float fDisMin = 2.5f;
    ...
    public float MinAngleOffset = 20f;
    ...
    public void Reset()
    {
        ...
        IsClockDir = Vector3.Cross(CameraFollowTarget.Instance.transform.forward,
            SceneObjectManager.Instance.CurSelectHero.CameraPoint.forward).y < 0;

        if (SceneObjectManager.Instance.CurSelectHero.CameraPoint.forward.normalized.magnitude == 0 || 
            CameraFollowTarget.Instance.transform.forward.normalized.magnitude == 0 ||
            SceneObjectManager.Instance.CurSelectHero.CameraPoint.forward.normalized == CameraFollowTarget.Instance.transform.forward.normalized)
            fTargetAngle = 0f;
        else
        {
            fTargetAngle = Mathf.Acos(Vector3.Dot(SceneObjectManager.Instance.CurSelectHero.CameraPoint.forward.normalized, CameraFollowTarget.Instance.transform.forward.normalized) / 
                                      (SceneObjectManager.Instance.CurSelectHero.CameraPoint.forward.normalized.magnitude * CameraFollowTarget.Instance.transform.forward.normalized.magnitude)) * Mathf.Rad2Deg;
        }

        if (Mathf.Sign(fTargetAngle) > 0)
        {
            fTargetAngle += MinAngleOffset;
        }
        else
        {
            fTargetAngle -= MinAngleOffset;
        ...
        if (Mathf.Abs(fTargetAngle) > 90)
        {
            fTargetAngle = Mathf.Sign(fTargetAngle) * 90f;
        }
    ...
    private float fDisTarget = 0;
    private void ResetCameraPostion()
    {
        if(SceneObjectManager.Instance.CurSelectHero == null)
            return;
        
        var enemy = SceneObjectManager.Instance.CurSelectHero.GetTarget;
        if (enemy == null)
        {
            return;
        }
        ...
        float cam_dis = 4f;
        var dis = Vector3.Distance(mon.GetPosition(), enemy.position);
        if (dis <= 2f)
        {
            cam_dis = fDisMin;
        }
        ...
        {
            cam_dis = fDisMin + (dis - 2) * (fDisMax - fDisMin) / (10 - 2);
        }
