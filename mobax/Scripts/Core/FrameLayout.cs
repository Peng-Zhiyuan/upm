...
[ExecuteInEditMode]
public class FrameLayout : MonoBehaviour, ILayoutGroup
{
    public enum FitType
    {
        /// <summary>
    ...
    private void Start()
    {
        ...
        if (Application.isPlaying)
        {
            if (this.topExpandUnsafeArea)
            {
                ...
                var h = UIEngine.Stuff.safeArea.TopNonSafeHight;
                var worldH = SpaceUtil.InverseTransformHeight(UIEngine.Stuff.CanvasTransform, h);
                var localH = SpaceUtil.TransformHeight(this.transform, worldH);
                this.rectTransform().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Top, -localH, selfSize.y + localH);
            }
            else if (this.bottomExpandUnsafeArea)
            {
                var selfSize = this.GetComponent<RectTransform>().rect.size;
                var h = UIEngine.Stuff.safeArea.BottomNonSafeHight;
                var worldH = SpaceUtil.InverseTransformHeight(UIEngine.Stuff.CanvasTransform, h);
                var localH = SpaceUtil.TransformHeight(this.transform, worldH);
                this.rectTransform().SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom, localH, selfSize.y + localH);
            }
