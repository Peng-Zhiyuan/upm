...
namespace BattleEngine.Logic
{
    ...
    public class Number : IValue
    {
        ...
        {
            return StrBuild.Instance.ToStringAppend("", m_Value.ToString(), "");
        }
    }
    ...
    public class OperationSum : IValue
    {
        ...
        public override string ToString()
        {
            return StrBuild.Instance.ToStringAppend("( ", string.Join(" + ", m_Values.Select(v => v.ToString()).ToArray()), " )");
        }
    }
    ...
    public class ExpressionParser
    {
        ...
        void SubstitudeBracket(ref string aExpression, int aIndex)
        {
            ...
            if (closing > aIndex + 1)
            {
                ...
                string sub = StrBuild.Instance.ToStringAppend("&", (m_BracketHeap.Count - 1).ToString(), ";");
                aExpression = StrBuild.Instance.ToStringAppend(aExpression.Substring(0, aIndex), sub, aExpression.Substring(closing + 1));
            }
        ...
        IValue Parse(string aExpression)
        {
            ...
            if (pPos > 0)
            {
                ...
                var m_FuncsData = m_Funcs.GetEnumerator();
                while (m_FuncsData.MoveNext())
                {
                    if (fName == m_FuncsData.Current.Key)
                    {
                        var inner = aExpression.Substring(m_FuncsData.Current.Key.Length);
                        ...
                        return new CustomFunction(m_FuncsData.Current.Key, m_FuncsData.Current.Value, parameters);
                    }
                }
            }
            var data = m_Consts.GetEnumerator();
            while (data.MoveNext())
            {
                if (aExpression == data.Current.Key)
                {
                    return new CustomFunction(data.Current.Key, (p) => data.Current.Value(), null);
                }
            }
                ...
                && index2b >= 2)
            {
                ...
                int bracketIndex;
                if (int.TryParse(inner, out bracketIndex)
                    && bracketIndex >= 0
                    && bracketIndex < m_BracketHeap.Count)
                {
