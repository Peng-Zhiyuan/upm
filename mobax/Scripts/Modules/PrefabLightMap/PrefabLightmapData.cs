...
public class PrefabLightmapData : MonoBehaviour {
	...
	static void GenerateLightmapInfo() {
		...
		Debug.LogError("prefabs count:"+ prefabs.Length);
		foreach (var instance in prefabs) {
			...
			var prefabObj = UnityEditor.PrefabUtility.GetCorrespondingObjectFromOriginalSource(gameObject);
			var targetPrefab = prefabObj as GameObject;
			if (targetPrefab != null) {
				
				bool result = false;
				string localPath = "Assets/Arts/" + targetPrefab.name + ".prefab";

				// Make sure the file name is unique, in case an existing Prefab has the same name.
				localPath = UnityEditor.AssetDatabase.GenerateUniqueAssetPath(localPath);
				UnityEditor.PrefabUtility.SaveAsPrefabAsset(gameObject, localPath, out result);
				Debug.LogError(targetPrefab.name+"=>result:" + result);
			}
		}
	}
	...
	static void GenerateLightmapInfo(GameObject root, List<RendererInfo> rendererInfos, List<Texture2D> lightmaps) {
		...
		foreach (MeshRenderer renderer in renderers) {
			Debug.LogError("renderer.lightmapIndex:"+ renderer.lightmapIndex);
			if (renderer.lightmapIndex != -1) {
				
				RendererInfo info = new RendererInfo();
				Debug.LogError("renderer.lightmapIndex:" + info.lightmapIndex);
