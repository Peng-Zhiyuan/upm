...
public class MediaPlayerCtrl : MonoBehaviour
{
    ...
    private MEDIAPLAYER_STATE m_bBufferState;
	...
	Call_SetUnityTexture (m_VideoTexture.GetNativeTextureID ());
			...
			var pos = Call_GetSeekPosition();
			Debug.Log($"【Media】compare: m_iCurrentSeekPosition: {m_iCurrentSeekPosition}, pos: {pos}");
            if( m_iCurrentSeekPosition == pos)
            {
                if (m_bBufferState == MEDIAPLAYER_STATE.PLAYING)
                {
	                m_bBufferState = MEDIAPLAYER_STATE.END;
                    
                    OnVideoBufferingEnd?.Invoke();
            ...
            {
	            m_bBufferState = MEDIAPLAYER_STATE.PLAYING;

	            OnVideoBuffering?.Invoke();
	            m_eventOnVideoBuffering?.Invoke();

            }

			m_iCurrentSeekPosition = pos;
		}
	...
	public void Load(string strFileName)
	{
		...
		m_bBufferState = MEDIAPLAYER_STATE.NOT_READY;
		m_iCurrentSeekPosition = 0;
	}
    ...
    private void Call_UnLoad()
	{
        ...
        if (m_CurrentState == MEDIAPLAYER_STATE.NOT_READY)
            return;
		m_CurrentState = MEDIAPLAYER_STATE.NOT_READY;
        bUnloadVideo = true;
	...
	private unsafe bool Call_Load(string strFileName, int iSeek)
	{
        ...
        m_typeGraphic = SystemInfo.graphicsDeviceType;


        loader = new Thread ( () => {

            if (bUnloadVideo == true)
                return;
    ...
    UnityEngine.Rendering.GraphicsDeviceType m_typeGraphic;
	...
	void LoadVideoPart2 ()
	{
...
#if UNITY_5
		if(m_typeGraphic == UnityEngine.Rendering.GraphicsDeviceType.Direct3D9)
		{
            bDirect9 = true;
        ...
        {
            pConvertContext = ffmpeg.sws_getContext(m_iWidth, m_iHeight, AVPixelFormat.AV_PIX_FMT_YUV420P,
    ...
    private void ThreadUpdate2()
    {
        while(true)
        {
            ...
            while (listVideo.Count > 30 || (bEnd == true && m_queuePacketVideo.Count == 0))
            {
                if (bUnloadVideo)
                    return;

                if ((bEnd == true && m_queuePacketVideo.Count == 0 && m_queuePacketSound.Count == 0))
                    return;
    ...
    private void Call_UpdateVideoTexture()
	{
        ...
        if ( m_CurrentState == MEDIAPLAYER_STATE.PLAYING && m_bIsFirstFrameReady == true  )
		{
            ...
            if (listVideo.Count > 0)
            {
                ...
                Debug.Log($"【Media】fCurrentSeekTime += Time.deltaTime * m_fSpeed -> {fCurrentSeekTime}");
            }
