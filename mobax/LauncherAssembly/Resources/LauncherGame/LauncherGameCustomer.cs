...
public class LauncherGameCustomer : MonoBehaviour
{
    public LauncherGameIcon ItemPrefab;
    public Transform ItemsNode;
    public LauncherCountdownBehaviour CountDown;
    public Image CountDownIcon;
    public LauncherShadowProgressBar ProgressBar;
    [HideInInspector]
    public Action<LauncherGameCustomer> OnLeave;

    private string[] _itemProvider;
    private LauncherGameIcon[] _icons;
    ...
    private void Update()
    {
        ...
        var total = LauncherGameData.CustomerTime * 1000;
        ...
        var stage = LauncherGameHelper.GetWaitStage(passedTime);
        if (_waitStage != stage)
        {
            _waitStage = stage;
    ...
    public string[] ItemProvider
    {
        set
        {
            ...
            if (_icons == null)
            {
                _icons = new LauncherGameIcon[_itemProvider.Length];
    ...
    {
        LauncherGameManager.RandomDemands(_itemProvider);
        ...
        if (!_stopped)
        {
            CountDown.SetEnd(LauncherGameData.CustomerTime, @"{0:ss}s");
        }
    }
