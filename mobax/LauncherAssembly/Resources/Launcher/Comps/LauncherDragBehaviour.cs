...
namespace UnityEngine.UI
{
    ...
    public class LauncherDragBehaviour : MonoBehaviour, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        public bool Dragable = true;
        public bool Clickable = true;

        public Action<LauncherDragBehaviour> onDown; // 触摸（或者MouseDown）的时候响应
        public Action<LauncherDragBehaviour> onClick;
        public Action<LauncherDragBehaviour> onDragStart;
        public Action<LauncherDragBehaviour> onDragging;
        public Action<LauncherDragBehaviour> onDragEnd;
        ...
        /** 是否点到目标物 */
        public (int index, Transform target) GetTargetByHitTestTargets(params Transform[] targets)
        {
            for (int i = 0; i < targets.Length; i++)
            {
                var target = targets[i];
                var rt = target.GetComponent<RectTransform>();
                var sizeDelta = this._draggingRt.sizeDelta;
