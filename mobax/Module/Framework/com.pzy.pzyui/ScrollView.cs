...
// pzy:
// 需要如此设置列表数据
//List<EntityInfo> dataList;
//private void ChangeScrollData(List<EntityInfo> dataList)
//{
//    this.dataList = dataList;
//    var count = dataList.Count;
//    this.ScrollView.ClearPrefab();
//    this.ScrollView.numItems = (uint)count;
//}
...
namespace UnityEngine.UI {
    ...
    public class ScrollView : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutGroup {
        ...
        [SerializeField]
        private bool m_PrefabDrag;
        public bool IfPrefabDrag { get { return m_PrefabDrag; } }
        ...
        public uint FixedCount 
        { 
            get 
            { 
                if(useFlexFixedCount2)
                {
                    return (uint)this.FlexFixedCount;
                }
                else
                {
                    return m_FixedCount;
        ...
        public bool useFlexFixedCount2 = true;

        public int FlexFixedCount
        {
            get
            {
                this.GetRectWidth(out var width);
                var length = width;
                length -= this.margin.left;
                length -= this.margin.right;
                var cellWidth = m_ItemSize[0];
                var spaceWidth = this.space.x;
                var a = cellWidth + spaceWidth;
                var count = (int)(length / a);
                var rest = length % a;
                if(rest >= cellWidth)
                {
                    count++;
        ...
        public Action<GameObject> onClickItem = null;

        /// <summary>
        /// pzy:
        /// 添加了普通的 Action 版本，方便在 js 中调用
        /// </summary>
        public Action<int, Transform> onItemRenderAction;
        ...
        //判断点击与滑动
        private float _ItemfingerTime;
        private Vector3 _ItemfristPos;
        ...
        public event Action OnPullRefresh; //scrollview 从下往上滑滑到底时执行的事件（上拉刷新）
        public event Action OnDropRefresh; //scrollview 从上往下滑滑到顶时执行的事件（下拉刷新）
        public void OnEndDrag (PointerEventData eventData) {
            ...
            //当contentBounds包围盒的最上点的坐标小于m_ViewBounds包围盒的最上方坐标，说明下拉幅度超过顶端，执行下拉刷新事件
            if (m_ContentBounds.max.y < m_ViewBounds.max.y) {
                OnDropRefresh?.Invoke ();
            }

            //当contentBounds包围盒的最下点的坐标小于m_ViewBounds包围盒的最下方坐标，说明上拉幅度超过底端，执行上拉刷新事件
        ...
        private void UpdatePosition (bool forceRender = false) {
            ...
            m_StartIdx = m_CurLine * (int) FixedCount;
            ...
            for (int i = m_StartIdx; i < m_EndIdx;) {
                for (int j = 0; j < FixedCount; j++) {
                    ...
                    if (!tmpContains.ContainsKey (virIdx)) {
                        ...
                        if (m_MotionType == MotionType.Vertical) {
                            Vector2 offset = new Vector2 ((j + 0.5f) * itemSize.x + j * m_Space.x, -(line + 0.5f) * itemSize.y - line * m_Space.y);
                            if (!m_Loop)
                                item.position = new Vector2 (margin.left, -margin.top) + offset;
                            else
                                item.position = new Vector2 (margin.left, 0) + offset;
                        } else {
                            Vector2 offset = new Vector2 ((line + 0.5f) * itemSize.x + line * m_Space.x, -(j + 0.5f) * itemSize.y - j * m_Space.y);
                            if (!m_Loop)
                                item.position = new Vector2 (margin.left, margin.top) + offset;
                            else
            ...
            for (int i = 0; i < num; i++) {
                ...
                if (item.renderable) {
                    ...
                    onItemRenderAction?.Invoke (item.actualIndex, item.transform);
                    //Debug.Log("actualIndex: " + item.actualIndex);
                }
                item.renderable = false;
            }
        ...
        void DisablePrfabIfNeed(GameObject prefab)
        {

            // 如果是在场景中，则隐藏预设
            // 如果是在 asset base 中，则不隐藏
            //rt.gameObject.SetActive (false);
            var scene = prefab.scene;
        ...
        private void SetPrefab (RectTransform rt) {
            ...
            DisablePrfabIfNeed(rt.gameObject);
            ...
            m_ItemSize.x = m_ItemSize.x > 0 ? m_ItemSize.x : size.x * scale.x;
            m_ItemSize.y = m_ItemSize.y > 0 ? m_ItemSize.y : size.y * scale.y;
        }

        /// <summary>
        /// pzy:
        /// 该名称有误，实际是移除 view，而非预制件
        /// </summary>
        public void ClearPrefab () {
            if (m_VirtualItems.Count > 0) {
                foreach (var item in m_VirtualItems) {
                    //item.Destroy ();
                    RecyleView (item.transform);
                }
                m_VirtualItems.Clear ();
            }
        }

        public List<ItemInfo> VirtualItemList
        {
            get
            {
                return this.m_VirtualItems;
            }
        }

        public Queue<Transform> viewPool = new Queue<Transform> ();
        // pzy:
        void RecyleView (Transform view) {
        ...
        Transform ReuseView () {
            if (viewPool.Count > 0) {
                var view = viewPool.Dequeue ();
                return view;
            }

            var go = GameObject.Instantiate (m_Prefab.gameObject) as GameObject;
            return go.transform;
        }

        public void InitVirtualItem () {
            foreach (var item in m_VirtualItems) {
                item.virtualIndex = int.MaxValue;
                item.actualIndex = int.MaxValue;
            }
        }

        private void SetNumItems (uint num) {
            // Debug.Log("setNumItems:" + num);
            m_NumItems = num;

            var isPlayer = Application.isPlaying;
            var hasInit = m_HasInit;
            var hasContent = m_Content != null;
            var hasPrefab = m_Prefab != null;
            var hasViewPort = m_ViewPort != null;
            if (!isPlayer || !hasInit || !hasContent || !hasPrefab || !hasViewPort)
                return;
            int count = m_VirtualItems.Count;
            int init = 0;
            if (m_Loop)
            ...
            if (m_MotionType == MotionType.Horizontal) {
                ...
                float y = margin.top + margin.bottom + m_ItemSize.y * FixedCount +
                    m_Space.y * Mathf.Clamp (FixedCount, 0, FixedCount - 1);
                ...
                float x = margin.left + margin.right + m_ItemSize.x * FixedCount +
                    m_Space.x * Mathf.Clamp (FixedCount, 0, FixedCount - 1);
        ...
        // 并且其中的延时在某些情况会引起 bug
        // 我尝试添加一下代码用来强制初始化
        public void ForceRebuildList()
        {
 
            DisablePrfabIfNeed(this.m_Prefab.gameObject);
            ...
            m_MaxInit = Mathf.CeilToInt(viewSize / cellSize) + 1;
            m_MaxInit *= (int)FixedCount;
            //Debug.Log("m_MaxInit:" + m_MaxInit);
        ...
        private IEnumerator RebuildList () {
            ...
            // Debug.Log("rebuildlist");
        }

        public event Action ListRebuilt;
        ...
        private int GetLoopIndex (float offset = 0) {
            ...
            // Debug.Log("pos" + pos +"-------top:" + m_Margin.top);
            if (m_Loop) {
                if (m_MotionType == MotionType.Horizontal) {
        ...
        private RectTransform CreateItem () {
            ...
            if (m_Prefab) {
                ...
                var obj = ReuseView ().gameObject;
                obj.transform.SetParent (m_Content, false);
                Vector3 _mv = obj.transform.localPosition;
                _mv.z = 0;
                obj.transform.localPosition = _mv;
                //obj.transform.localScale = Vector3.one;
                if (onClickItem != null) {
                    EventTriggerListener.Get (obj).onDown = OnItemDown;
                    EventTriggerListener.Get (obj).onUp = OnItemUp;
                    if (!m_PrefabDrag) {
                        EventTriggerListener.Get (obj).onDrag = OnDrag;
                        EventTriggerListener.Get (obj).onBeginDrag = OnBeginDrag;
                        EventTriggerListener.Get (obj).onInitializePotentialDrag = OnInitializePotentialDrag;
                        EventTriggerListener.Get (obj).onEndDrag = OnEndDrag;
                    }
                }
        ...
        private void OnItemDown (GameObject go) {
            _ItemfristPos = Input.mousePosition;
            _ItemfingerTime = Time.time;
        }
        private void OnItemUp (GameObject obj) {
            if (Vector2.Distance (_ItemfristPos, Input.mousePosition) < 0.5f) {
                _ItemfingerTime = 0;
                _ItemfristPos = Vector3.zero;
                onClickItem?.Invoke (obj);
                return;
            }
            if (Vector2.Distance (_ItemfristPos, Input.mousePosition) < 5f) {
                _ItemfingerTime = Time.time - _ItemfingerTime;
                if (_ItemfingerTime < 0.3f) {
                    _ItemfingerTime = 0;
                    _ItemfristPos = Vector3.zero;
                    onClickItem?.Invoke (obj);
                }
