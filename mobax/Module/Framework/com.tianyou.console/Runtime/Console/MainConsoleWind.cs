...
public class MainConsoleWind : MonoBehaviour
{
    ...
    static List<Type> _commandTypeList;
    static List<Type> CommandTypeList
    {
        get
        {
            if (_commandTypeList == null)
            {
                //var types = ReflectionUtil.GetSubClassesInAllAssemblies<BuildinCommands>();
                var types = ReflectionUtil.GetAttributedClassesInAllAssemblies<ConsoleCommandsAttribute>();
                _commandTypeList = types;
            }
            return _commandTypeList;
        }
    }

    static Type GetCmdOriginClass(string method)
    {
        var types = CommandTypeList;
    ...
    public static void GetParamCount(string funcName, out int min, out int max)
    {
        if (!ConstainsFunction(funcName))
        {
            throw new Exception($"[MainConsoleWind] Method {funcName} do not exist");
        }
        var parameters = GetCmdMethod(funcName).GetParameters();
        ...
        for (var i = total - 1; i >= 0; i--)
        {
            ...
            {
                // 这个optional的也要传，不然还是报错，所以注释掉了
                // ++optionalNum;
            }
        }

        min = total - optionalNum;
        max = infinite ? Int32.MaxValue : total;
    }
    ...
    public void InputCmd(string cmd)
    {
        try
        {
            if (inputLogs.Count == 0 || cmd != inputLogs[inputLogs.Count - 1].Trim())
            {
                inputLogs.Add(cmd);
                // 超过一万行记录就清除
                RefreshInputLog();
        ...
        var mainCmd = cmds[0];
        if (!Debug.isDebugBuild && developOnlyFunctions.Contains(mainCmd))
        {
            ShowString($" -Command : [{mainCmd}] is a development build only command.", Color.red);
            return;
        }

        if (!ConstainsFunction(mainCmd))
        {
            NextGuessCmd = GetSimilarityCommand(mainCmd)[0];
            ShowString($" -Command : [{mainCmd}] do not exist. Are you looking for [{NextGuessCmd}]?", Color.red);
            return;
        }

        NextGuessCmd = "";

        if (banFunctions.Contains(mainCmd))
        {
            ShowString($" -Command : [{mainCmd}] is not valid.", Color.red);
            return;
        }

        var inputParamCount = cmds.Count - 1;
        GetParamCount(mainCmd, out var minCount, out var maxCount);
        if (inputParamCount < minCount || inputParamCount > maxCount)
        {
            ShowString($" -Command : [{mainCmd}] param number wrong.", Color.red);
            ShowHelp(mainCmd, false);
            return;
        }
