...
{
    public static void FbxModelNormalSmoothTool(Object[] selectionObjects, int storeUvChannel, string filterKey)
    {
        ...
        foreach (Object asset in selectionObjects)
        {
            ...
            Debug.Log("Export fbx start:" + assetPath);
            ...
            if (assetName.Contains(filterKey))
            {
                continue;
            }
           ...
           // if (!fbxExporter.Initialize(assetPath.Substring(0, assetPath.Length - 4) + "Smooth.fbx", -1, fbxIOSettings))
            if (!fbxExporter.Initialize(assetPath.Substring(0, assetPath.Length - 4) + ".fbx", -1, fbxIOSettings))
            {
                Debug.Log(fbxExporter.GetStatus().GetErrorString());
                continue;
            }
            Debug.Log("Export fbx suc:"+ assetPath);
            smoothedCount++;
    ...
    static FbxVector4[] GetMeshTangents(FbxMesh fbxMesh)
    {
        ...
        //Debug.LogError("tangents:"+ tangents.Length + "polygonTangents:"+ polygonTangents.Length);
        int index = 0;
        for (int i = 0; i < polygonCount; i++)
        {
            for (int j = 0; j < fbxMesh.GetPolygonSize(i); j++)
            {
                if (j >= 4)
                {
                    Debug.LogError("index:" + (index + 1) + "    i:" + i + "   j:" + j);
                    continue;
                }
...
public class FbxMeshSmoothToolWindow : EditorWindow
{
    ...
    private void OnGUI()
    {
        ...
        {
            FbxMeshNormalProcessor.FbxModelNormalSmoothTool(objects, selectedChannel + 2, "@");
            Close();
        }
        EditorGUILayout.Space();
