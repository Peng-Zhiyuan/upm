...
namespace Pathfinding.Examples {
	...
	public class MineBotAI : AIPath {
		...
		protected override int OnUpgradeSerializedData (int version, bool unityThread) {
			if (unityThread) {
				var components = gameObject.GetComponents<CoreComponent>();
				...
				foreach (System.Type newType in new [] { typeof(AIPath), typeof(MineBotAnimation) }) {
					...
					for (int i = components.Length - 1; i > index; i--)
					{
						throw new System.Exception("[MineBotAi] core engine not support move component order");
						//UnityEditorInternal.ComponentUtility.MoveComponentUp(newComp);
					}
				}
				throw new System.Exception("[MineBotAi] core engine not support destory core component");
