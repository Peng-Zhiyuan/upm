...
namespace Pathfinding.Examples {
	...
	public class BezierMover : VersionedMonoBehaviour {
		...
		Vector3 Position (float t) {
			...
			return AstarSplines.CatmullRom(points[(pt-1+c)%c].position, points[pt].position, points[(pt+1)%c].position, points[(pt+2)%c].position, (Fixed)t - Mathf.FloorToInt(t));
		}
