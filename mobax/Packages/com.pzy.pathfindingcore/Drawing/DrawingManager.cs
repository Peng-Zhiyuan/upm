...
namespace PathfindingCore.Drawing {
	...
	public static class GizmoContext {
#if UNITY_EDITOR
		static CoreTransform activeTransform;
#endif

		static HashSet<CoreTransform> selectedTransforms = new HashSet<CoreTransform>();
		...
		static public CoreTransform SelectedActiveCoreTransform
        {
			get
            {
				...
				return debuger?.co.GetComponent<CoreTransform>();
		...
		static public CoreTransform[] SelectedCoreTransform
		{
			get
			{
				...
				foreach(var one in monoTransform)
                {
					...
					var coreTransform = debuger?.co.GetComponent<CoreTransform>();
		...
		internal static void Refresh () {
			...
			activeTransform = SelectedActiveCoreTransform;

			selectedTransforms.Clear();
			//var topLevel = Selection.transforms;
			var topLevel = SelectedCoreTransform;
		...
		/// </summary>
		public static bool InSelection (CoreComponent c) {
		...
		/// This is a deep selection: even children of selected transforms are considered to be selected.
		/// </summary>
		public static bool InSelection (CoreTransform tr) {
			if (!drawingGizmos) throw new System.Exception("Can only be used inside the ALINE library's gizmo drawing functions.");
			var leaf = tr;
			while (tr != null) {
		...
		/// </summary>
		public static bool InActiveSelection (CoreComponent c) {
			if (!drawingGizmos) throw new System.Exception("Can only be used inside the ALINE library's gizmo drawing functions.");
		...
		/// The active selection is the GameObject that is currently visible in the inspector.
		/// </summary>
		public static bool InActiveSelection (CoreTransform tr) {
#if UNITY_EDITOR
			if (!drawingGizmos) throw new System.Exception("Can only be used inside the ALINE library's gizmo drawing functions.");
	...
	[ExecuteAlways]
	[AddComponentMenu("")]
	public class DrawingManager : CoreComponent {
		...
		public static void Init () 
		{
			...
			//};
			//var go = new CoreObject("RetainedGizmos");
			var go = CoreEngine.lastestInstance.Create("RetainedGizmos");
			go.dontDestoryFlag = true;
		...
		void DelayedDestroy () {
			...
			if (!this.coreObject.isMarkedDestoryed) CoreObject.DestroyImmediate(this.coreObject);
		}
#endif
		...
		void RemoveDestroyedGizmoDrawers () {
			for (int i = gizmoDrawers.Count - 1; i >= 0; i--) {
				...
				var mono = gizmoDrawers[i] as CoreComponent;
				//if (!mono || (mono.hideFlags & HideFlags.HideInHierarchy) == HideFlags.HideInHierarchy) 
				if (mono.coreObject == null || mono.coreObject.isMarkedDestoryed)
				{
		...
		void DrawGizmos (bool usingRenderPipeline) {
			...
			if (!usingRenderPipeline) {
				...
				for (int i = gizmoDrawers.Count - 1; i >= 0; i--) {
					...
					if (!typeToGizmosEnabled.ContainsKey(tp) && (gizmoDrawers[i] as CoreComponent).isActiveAndEnabled)
					{
			...
			try {
				...
				if (usingRenderPipeline) {
					...
					for (int i = gizmoDrawers.Count - 1; i >= 0; i--) {
						...
						var co = gizmoDrawers[i] as CoreObject;
						var debugerGo = co?.debbugerGameObject;
						var disabledDueToIsolationMode = false;
						if(debugerGo != null)
							disabledDueToIsolationMode = isInNonMainStage && StageUtility.GetStage(debugerGo as GameObject) != currentStage;
#else
						var disabledDueToIsolationMode = false;
#endif
						//if ((gizmoDrawers[i] as MonoBehaviour).isActiveAndEnabled && !disabledDueToIsolationMode) {
						if ((gizmoDrawers[i] as CoreComponent).isActiveAndEnabled && !disabledDueToIsolationMode)
						{
							try {
								gizmoDrawers[i].DrawGizmos();
							} catch (System.Exception e) {
								Debug.LogException(e, (gizmoDrawers[i] as CoreComponent).coreObject.debbugerGameObject as GameObject);
							}
						}
					}
				} else {
					for (int i = gizmoDrawers.Count - 1; i >= 0; i--) {
						...
						// True if the scene is in isolation mode (e.g. focusing on a single prefab) and this object is not part of that sub-stage
						//var disabledDueToIsolationMode = isInNonMainStage && StageUtility.GetStage((gizmoDrawers[i] as MonoBehaviour).gameObject) != currentStage;
						var disabledDueToIsolationMode = isInNonMainStage && StageUtility.GetStage((gizmoDrawers[i] as CoreComponent).coreObject.debbugerGameObject as GameObject) != currentStage;
#else
						var disabledDueToIsolationMode = false;
#endif
...
#if UNITY_EDITOR
			var a = Handles.ShouldRenderGizmos();
			var b = drawToAllCameras;
			bool drawGizmos = a || b;
#else
