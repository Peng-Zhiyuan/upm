...
namespace PathfindingCore {
	...
	public abstract class NavmeshBase : NavGraph, INavmesh, INavmeshHolder, ITransformedGraph
		...
		///
		/// You can also control this using a <see cref="PathfindingCore.NNConstraint.distanceXZ field on an NNConstraint"/>.
		/// </summary>
		[JsonMember]
		...
		protected override void OnDestroy () {
			...
			if (tiles != null) {
				for (int i = 0; i < tiles.Length; i++) {
					PathfindingCore.Util.ObjectPool<BBTree>.Release(ref tiles[i].bbTree);
				}
			}
		}
		...
		protected struct JobConnectTiles : Unity.Jobs.IJob {
		...
		protected static void CreateNodes (NavmeshTile tile, int[] tris, int tileIndex, uint graphIndex, uint[] tags, AstarPathCore astar, uint initialPenalty, bool recalculateNormals) {
			var nodes = tile.nodes;
		...
		/// <summary>Cached <see cref="PathfindingCore.NNConstraint.None"/> with distanceXZ=true to reduce allocations</summary>
		...
		void CreateNavmeshSurfaceVisualization (NavmeshTile[] tiles, int startTile, int endTile, GraphGizmoHelper helper) {
			...
			for (int i = startTile; i < endTile; i++) {
				...
				if (showMeshSurface)
				{
					for (int j = 0; j < tile.nodes.Length; j++)
					{
						...
						var drawingText = $"{graphIndex}:{nodeIndex}";
						var vp = (Vector3)postion;
						helper.builder.Label2D(vp, drawingText, 14, LabelAlignment.Center) ;
			...
			if (showMeshSurface)
			{
				helper.DrawTriangles(vertices, colors, numNodes);
				
			}
			if (showMeshOutline)
			{
				helper.DrawWireTriangles(vertices, colors, numNodes);
			}
