...
namespace PathfindingCore {
	...
	public class TriangleMeshNode : MeshNode {
		...
		public TriangleMeshNode (AstarPathCore astar) {
		...
		public override Vector3 ClosestPointOnNode (Vector3 p) {
			...
			GetVertices(out a, out b, out c);
			return PathfindingCore.Polygon.ClosestPointOnTriangle((Vector3)a, (Vector3)b, (Vector3)c, p);
		}

		/// <summary>
		/// Closest point on the node when seen from above.
		/// This method is mostly for internal use as the <see cref="PathfindingCore.NavmeshBase.Linecast"/> methods use it.
		///
		...
		internal Int3 ClosestPointOnNodeXZInGraphSpace (Vector3 p) {
			...
			var closest = PathfindingCore.Polygon.ClosestPointOnTriangleXZ((Vector3)a, (Vector3)b, (Vector3)c, p);
		...
		public bool GetPortal (GraphNode toNode, System.Collections.Generic.List<Vector3> left, System.Collections.Generic.List<Vector3> right, bool backwards, out int aIndex, out int bIndex) {
			...
			// pzy:
			// 这一行注释掉了
			// 原本会在两个节点属于不同 graph 时直接返回 false
			// 但是这会错误处理 graph 拼接的情况
			// 会导致寻路不正常
