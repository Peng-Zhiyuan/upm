...
namespace PathfindingCore {
	...
	public class PointGraph : NavGraph
		...
		///
		/// See: <see cref="PathfindingCore.StartEndModifier.exactEndPoint"/>
		/// </summary>
		...
		NNInfoInternal GetNearestInternal (Vector3 position, NNConstraint constraint, bool fastCheck) {
			...
			float maxDistSqr = constraint == null || constraint.constrainDistance ? this.active.maxNearestNodeDistanceSqr : float.PositiveInfinity;
		...
		void IUpdatableGraph.UpdateArea (GraphUpdateObject guo) {
			...
			if (guo.updatePhysics) {
				...
				List<Connection> tmpList = PathfindingCore.Util.ListPool<Connection>.Claim();
				...
				PathfindingCore.Util.ListPool<Connection>.Release(ref tmpList);
			}
		}
