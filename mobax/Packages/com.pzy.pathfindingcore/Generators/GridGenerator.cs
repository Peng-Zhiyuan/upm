...
namespace PathfindingCore {
	...
	using PathfindingCore.Jobs.Grid;
	using Unity.Burst;
	...
	///
	/// See: <see cref="PathfindingCore.GraphCollision"/> for documentation on the 'Height Testing' and 'Collision Testing' sections
	...
	public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph
		...
		public override NNInfoInternal GetNearestForce (Vector3 position, NNConstraint constraint) {
			...
			float maxDist = constraint == null || constraint.constrainDistance ? this.active.maxNearestNodeDistance : float.PositiveInfinity;
		...
		public struct GridGraphScanData {
			...
			public JobHandle AssignToNodes (AstarPathCore astar, GridNodeBase[] nodes, int3 nodeArrayBounds, IntBounds writeMask, uint graphIndex, JobHandle nodesDependsOn, NativeArray<float4> nodeSurfaceNormals) {
				// This is a managed type, we need to trick Unity to allow this inside of a job
				var nodesHandle = System.Runtime.InteropServices.GCHandle.Alloc(nodes);
		...
		IEnumerator<JobHandle> UpdateAreaBurstCoroutine (GridNodeBase[] newNodes, int3 nodeArrayBounds, IntRect rect, JobDependencyTracker dependencyTracker, JobHandle nodesDependsOn, Allocator allocationMethod, RecalculationMode recalculationMode, GraphUpdateObject graphUpdateObject) {
			...
			var astar = this.active;

			yield return context.data.AssignToNodes(astar, newNodes, newNodeArrayBounds, writeMaskBounds, graphIndex, new JobHandle(), newNodeSurfaceNormals);
		...
		public void ErodeWalkableArea (int xmin, int zmin, int xmax, int zmax) {
			if (erosionUseTags) {
				if (erodeIterations+erosionFirstTag > 31) {
					Debug.LogError("Too few tags available for "+erodeIterations+" erode iterations and starting with tag " + erosionFirstTag + " (erodeIterations+erosionFirstTag > 31)", active.coreObject.debbugerGameObject as UnityEngine.Object);
					return;
				}
				if (erosionFirstTag <= 0) {
					Debug.LogError("First erosion tag must be greater or equal to 1", active.coreObject.debbugerGameObject as UnityEngine.Object);
					return;
				}
			}
			...
			PathfindingCore.Util.ListPool<GraphNode>.Release(ref nodesInRect);
		}
