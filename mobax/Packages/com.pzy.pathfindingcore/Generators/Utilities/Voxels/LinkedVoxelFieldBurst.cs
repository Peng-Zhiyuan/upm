...
namespace PathfindingCore.Voxels.Burst {
	...
	public struct LinkedVoxelField {
		...
		public void Dispose () {
			linkedSpans.Dispose();
			removedStack.Dispose();
			linkedCellMinMax.Dispose();
		}
		...
		public void AddLinkedSpan (int index, uint bottom, uint top, int area, int voxelWalkableClimb, int objectID) {
			...
			// A threshold of 1 seems to work fine usually, but we use a slightly higher one
			// to guard against potential floating point errors and other nasty things.
			// Note that this threshold is in the voxel y resolution (which is very high)
			const int SPAN_AREA_MERGE_THRESHOLD = 4;
			...
			while (index != -1) {
				...
				if (current.bottom > top) {
					...
					if (math.abs((int)top - (int)current.top) < SPAN_AREA_MERGE_THRESHOLD && (area == CompactVoxelField.UnwalkableArea || current.area == CompactVoxelField.UnwalkableArea)) {
