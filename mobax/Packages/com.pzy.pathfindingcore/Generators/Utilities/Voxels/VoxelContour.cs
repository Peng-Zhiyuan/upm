...
namespace PathfindingCore.Voxels {
	public partial class Voxelize {
		public void BuildContours (float maxError, int maxEdgeLength, VoxelContourSet cset, int buildFlags) {
			...
			List<int> verts = PathfindingCore.Util.ListPool<int>.Claim(256);//new List<int> (256);
			List<int> simplified = PathfindingCore.Util.ListPool<int>.Claim(64);//new List<int> (64);
			...
			for (int z = 0; z < wd; z += voxelArea.width) {
				for (int x = 0; x < voxelArea.width; x++) {
					...
					for (int i = (int)c.index, ci = (int)(c.index+c.count); i < ci; i++) {
						...
						contour.verts = PathfindingCore.Util.ArrayPool<int>.Claim(simplified.Count);//simplified.ToArray ();
			...
			PathfindingCore.Util.ListPool<int>.Release(ref verts);
			PathfindingCore.Util.ListPool<int>.Release(ref simplified);
		...
		static void ReleaseContours (VoxelContourSet cset) {
			for (int i = 0; i < cset.conts.Count; i++) {
				...
				PathfindingCore.Util.ArrayPool<int>.Release(ref cont.verts);
				PathfindingCore.Util.ArrayPool<int>.Release(ref cont.rverts);
			}
			cset.conts = null;
		}
		...
		public static bool MergeContours (ref VoxelContour ca, ref VoxelContour cb, int ia, int ib) {
			...
			int[] verts = PathfindingCore.Util.ArrayPool<int>.Claim(maxVerts*4);
			...
			PathfindingCore.Util.ArrayPool<int>.Release(ref ca.verts);
			PathfindingCore.Util.ArrayPool<int>.Release(ref cb.verts);

			ca.verts = verts;
			ca.nverts = nv;

			cb.verts = PathfindingCore.Util.ArrayPool<int>.Claim(0);
			cb.nverts = 0;
