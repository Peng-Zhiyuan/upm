...
namespace PathfindingCore {
	...
	public class EuclideanEmbedding {
		...
		void GetClosestWalkableNodesToChildrenRecursively (AstarPathCore astar, Transform tr, List<GraphNode> nodes) {
			foreach (Transform ch in tr) {
				var info = astar.GetNearest(ch.position, NNConstraint.Default);
				if (info.node != null && info.node.Walkable) {
					nodes.Add(info.node);
				}

				GetClosestWalkableNodesToChildrenRecursively(astar, ch, nodes);
			}
		}
		...
		/// </summary>
		void PickNRandomNodes (AstarPathCore astar, int count, List<GraphNode> buffer) {
			int n = 0;
		...
		GraphNode PickAnyWalkableNode (AstarPathCore astar) {
			var graphs = astar.graphs;
		...
		public void RecalculatePivots (AstarPathCore astar) {
			...
			var pivotList = PathfindingCore.Util.ListPool<GraphNode>.Claim();
			...
			case HeuristicOptimizationMode.Custom:
				...
				GetClosestWalkableNodesToChildrenRecursively(astar, pivotPointRoot, pivotList);
				break;
			case HeuristicOptimizationMode.Random:
				PickNRandomNodes(astar, spreadOutCount, pivotList);
				break;
			case HeuristicOptimizationMode.RandomSpreadOut:
				...
				if (pivotList.Count == 0) {
					GraphNode first = PickAnyWalkableNode(astar);

					if (first != null) {
						pivotList.Add(first);
					} else {
						Debug.LogError("Could not find any walkable node in any of the graphs.");
						PathfindingCore.Util.ListPool<GraphNode>.Release(ref pivotList);
						return;
					}
				}
		...
		public void RecalculateCosts (AstarPathCore astar) {
			if (pivots == null) RecalculatePivots(astar);
			...
			OnPathDelegate onComplete = (Path path) => {
				...
				if (numComplete == pivotCount) {
					// Last completed path
					ApplyGridGraphEndpointSpecialCase(astar);
				}
			};
			...
			startCostCalculation = (int pivotIndex) => {
				...
				floodPath = FloodPath.Construct(astar, pivot, onComplete);
				...
				//AstarPathCore.StartPath(floodPath, true);
				astar.StartPath(floodPath, true);
			};
		...
		/// </summary>
		void ApplyGridGraphEndpointSpecialCase (AstarPathCore astar) {
