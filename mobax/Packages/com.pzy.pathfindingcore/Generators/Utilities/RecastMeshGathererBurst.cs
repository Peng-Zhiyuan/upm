...
namespace PathfindingCore.Recast {
	using System;
	...
	internal class RecastMeshGathererBurst {
		...
		public struct MeshCollection {
			...
			public void Dispose () {
				for (int i = 0; i < vertexBuffers.Count; i++) {
					...
					triangleBuffers[i].Dispose();
				}
				ListPool<NativeArray<Vector3> >.Release(ref vertexBuffers);
				ListPool<NativeArray<int> >.Release(ref triangleBuffers);
				meshes.Dispose();
			}
		}
		...
		bool MeshFilterShouldBeIncluded (MeshFilter filter) {
			if (filter.TryGetComponent<Renderer>(out Renderer rend)) {
				if (filter.sharedMesh != null && rend.enabled && (((1 << filter.gameObject.layer) & mask) != 0 || tagMask.Contains(filter.tag))) {
					if (!(filter.TryGetComponent<RecastMeshObj>(out RecastMeshObj rmo) && rmo.IsEnabled)) {
						return true;
					}
				}
			}
		...
		public void CollectRecastMeshObjs () {
			...
			for (int i = 0; i < buffer.Count; i++) {
				...
				if (filter != null && filter.TryGetComponent<Renderer>(out Renderer rend) && filter.sharedMesh != null) {
					...
					if (coll == null) {
						Debug.LogError("RecastMeshObject ("+buffer[i].coreObject.Name +") didn't have a collider or MeshFilter+Renderer attached", buffer[i].coreObject.debbugerGameObject as UnityEngine.Object);
						continue;
					}
				...
				if (!cachedTreePrefabs.TryGetValue(prot.prefab, out treeInfo)) {
					...
					for (int j = 0; j < colliders.Count; j++) {
						...
						if (GetColliderMesh(colliders[j], rootMatrixInv * colliders[j].transform.localToWorldMatrix) is GatheredMesh mesh) {
							if (colliders[j].gameObject.TryGetComponent<RecastMeshObj>(out RecastMeshObj recastMeshObj) && recastMeshObj.IsEnabled) {
		...
		bool ShouldIncludeCollider (Collider collider) {
			return (((mask >> collider.gameObject.layer) & 1) != 0 || tagMask.Contains(collider.tag)) && collider.enabled && !collider.isTrigger && collider.bounds.Intersects(bounds) && !(collider.TryGetComponent<RecastMeshObj>(out RecastMeshObj rmo) && rmo.IsEnabled);
		}
		...
		public void CollectColliderMeshes () {
			...
			do {
				...
				numColliders = Physics.OverlapBoxNonAlloc(bounds.center, bounds.extents, colliderBuffer, Quaternion.identity, ~0, QueryTriggerInteraction.Ignore);
