		...
		public void CollectRecastMeshObjs (List<RasterizationMesh> buffer) {
			...
			for (int i = 0; i < buffer2.Count; i++) {
				...
				if (filter != null && rend != null && filter.sharedMesh != null) {
					...
					if (coll == null) {
						Debug.LogError("RecastMeshObject ("+buffer2[i].coreObject.Name +") didn't have a collider or MeshFilter+Renderer attached", buffer2[i].coreObject.debbugerGameObject as UnityEngine.Object);
						continue;
					}
