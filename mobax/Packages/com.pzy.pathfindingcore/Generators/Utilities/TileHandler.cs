...
namespace PathfindingCore.Util {
	...
	/// See: <see cref="PathfindingCore.NavmeshUpdates"/>
	/// </summary>
	public class TileHandler {
		...
		/// This is updated by the <see cref="PathfindingCore.NavmeshUpdates"/> class.
		/// See: <see cref="PathfindingCore.NavmeshUpdates"/>
		/// </summary>
		...
		/// </summary>
		readonly PathfindingCore.Voxels.Int3PolygonClipper simpleClipper;
		...
		public void OnRecalculatedTiles (NavmeshTile[] recalculatedTiles) {
			...
			var astar = this.graph.active;
			StartBatchLoad(astar);
		...
		/// </summary>
		public void StartBatchLoad (AstarPathCore astar) {
			batchDepth++;
			if (batchDepth > 1) return;

			astar.AddWorkItem(new AstarWorkItem(force => {
				graph.StartBatchTileUpdate();
				return true;
			}));
		}

		public void EndBatchLoad (AstarPathCore astar) {
			...
			astar.AddWorkItem(new AstarWorkItem((ctx, force) => {
		...
		CuttingResult CutPoly (Int3[] verts, int[] tris, Int3[] extraShape, GraphTransform graphTransform, IntRect tiles, CutMode mode = CutMode.CutAll | CutMode.CutDual, int perturbate = -1) {
			...
			if (navmeshCuts.Count == 0 && navmeshAdds.Count == 0 && (mode & ~(CutMode.CutAll | CutMode.CutDual)) == 0 && (mode & CutMode.CutAll) != 0) {
				...
				var polypoints = ListPool<Pathfinding.Poly2Tri.PolygonPoint>.Claim();

				var clipResult = new Pathfinding.ClipperLib.PolyTree();
				var intermediateClipResult = ListPool<List<IntPoint> >.Claim();
				var polyCache = StackPool<Pathfinding.Poly2Tri.Polygon>.Claim();
							...
							for (int exp = 0; exp < clipResult.ChildCount; exp++) {
								...
								if (holes.Count == 0 && outer.Count == 3 && meshIndex == -1) {
									for (int i = 0; i < 3; i++) {
										...
										p.y = PathfindingCore.Polygon.SampleYCoordinateInTriangle(tp1, tp2, tp3, p);

										outtris.Add(outverts.Count);
										outverts.Add(p);
									}
								} else {
									Pathfinding.Poly2Tri.Polygon polygonToTriangulate = null;
									...
									while (contour != null) {
										...
										Pathfinding.Poly2Tri.Polygon contourPolygon = null;
										if (polyCache.Count > 0) {
											contourPolygon = polyCache.Pop();
											contourPolygon.AddPoints(polypoints);
										} else {
											contourPolygon = new Pathfinding.Poly2Tri.Polygon(polypoints);
										}
									...
									} catch (Pathfinding.Poly2Tri.PointOnEdgeException) {
									...
									try {
										for (int i = 0; i < polygonToTriangulate.Triangles.Count; i++) {
											Pathfinding.Poly2Tri.DelaunayTriangle t = polygonToTriangulate.Triangles[i];
				...
				StackPool<Pathfinding.Poly2Tri.Polygon>.Release(polyCache);
				ListPool<List<IntPoint> >.Release(ref intermediateClipResult);
				ListPool<IntPoint>.Release(ref poly);
				ListPool<Pathfinding.Poly2Tri.PolygonPoint>.Release(ref polypoints);
			}
			...
			PathfindingCore.Polygon.CompressMesh(outverts, outtris, out result.verts, out result.tris);
		...
		static void PoolPolygon (Pathfinding.Poly2Tri.Polygon polygon, Stack<Pathfinding.Poly2Tri.Polygon> pool) {
		...
		public void ClearTile (AstarPathCore astar, int x, int z) {
			if (astar == null) return;

			if (x < 0 || z < 0 || x >= tileXCount || z >= tileZCount) return;

			astar.AddWorkItem(new AstarWorkItem((context, force) => {
