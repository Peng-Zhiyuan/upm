...
namespace PathfindingCore {
	...
	public class MultiTargetPath : ABPath {
		...
		public static MultiTargetPath Construct (AstarPathCore astar, Vector3[] startPoints, Vector3 target, OnPathDelegate[] callbackDelegates, OnPathDelegate callback = null) {
			MultiTargetPath p = Construct(astar, target, startPoints, callbackDelegates, callback);

			p.inverted = true;
			return p;
		}

		public static MultiTargetPath Construct (AstarPathCore astar, Vector3 start, Vector3[] targets, OnPathDelegate[] callbackDelegates, OnPathDelegate callback = null) {
			var p = PathPool.GetPath<MultiTargetPath>(astar);
		...
		protected override void Prepare () {
			...
			for (int i = 0; i < targetPoints.Length; i++) {
				var endNNInfo = this.astar.GetNearest(targetPoints[i], nnConstraint);
