...
namespace PathfindingCore {
	...
	public class FloodPath : Path {
		...
		public static FloodPath Construct (AstarPathCore astar, Vector3 start, OnPathDelegate callback = null) {
			var p = PathPool.GetPath<FloodPath>(astar);

			p.Setup(start, callback);
			return p;
		}

		public static FloodPath Construct (AstarPathCore astar, GraphNode start, OnPathDelegate callback = null) {
		...
		protected override void Prepare () {
			...
			if (startNode == null) {
				...
				var startNNInfo  = this.astar.GetNearest(originalStartPoint, nnConstraint);
