...
namespace PathfindingCore {
	...
	public class ABPath : Path {
		...
		/// </summary>
		public static ABPath Construct (AstarPathCore astar, Vector3 start, Vector3 end, OnPathDelegate callback = null) {
			var p = PathPool.GetPath<ABPath>(astar);
		...
		/// </code>
		/// </summary>
		public static ABPath FakePath (AstarPathCore astar, List<Vector3> vectorPath, List<GraphNode> nodePath = null) {
			var path = PathPool.GetPath<ABPath>(astar);
...
#if !ASTAR_NO_GRID_GRAPH
		/// <summary>Cached <see cref="PathfindingCore.NNConstraint.None"/> to reduce allocations</summary>
		...
		protected virtual bool EndPointGridGraphSpecialCase (GraphNode closestWalkableEndNode) {
			...
			if (gridNode != null) {
				...
				var endNNInfo2 = this.astar.GetNearest(originalEndPoint, NNConstraintNone);
		...
		protected override void Prepare () {
			...
			var startNNInfo  = this.astar.GetNearest(startPoint, nnConstraint);
			...
			if (hasEndPoint) 
			{
				...
				var endNNInfo = this.astar.GetNearest(endPoint, nnConstraint);
