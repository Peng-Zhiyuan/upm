...
namespace PathfindingCore.RVO {
	...
	public class RVOController : VersionedMonoBehaviour {
		...
		/// It is primarily used by the <see cref="PathfindingCore.RVO.RVODestinationCrowdedBehavior"/>.
		/// </summary>
		...
		///
		/// The position of the agent is taken from the attached movement script's position (see <see cref="PathfindingCore.IAstarAI.position)"/> or if none is attached then transform.position.
		/// </summary>
		...
		public Vector3 CalculateMovementDelta (float deltaTime) {
			...
			var delta = movementPlane.ToPlane(rvoAgent.CalculatedTargetPoint - (Vector3)(ai != null ? ai.position : tr.position));
		...
		public void Move (Vector3 velocity) {
			...
			rvoAgent.SetTarget((Vector3)(ai != null ? ai.position : tr.position) + velocity, speed, speed);
		...
		public override void DrawGizmos () {
			...
			if (ai == null) {
				...
				var scale = tr.localScale;
				if (movementPlaneMode == MovementPlane.XY) {
					throw new System.Exception("[RVOController] not implement yet");
