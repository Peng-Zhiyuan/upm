...
namespace PathfindingCore {
	...
	///
	/// This modifier is included in the <see cref="PathfindingCore.Seeker"/> component and is always used if you are using a Seeker.
	...
	public class StartEndModifier : PathModifier {
		...
		public enum Exactness {
			...
			///
			/// Note: If you are using this mode with a <see cref="PathfindingCore.PointGraph"/> you probably also want to use the <see cref="PathfindingCore.PointGraph.NodeDistanceMode Connection"/> for <see cref="PathfindingCore.PointGraph.nearestNodeDistanceMode"/>.
			///
			/// [Open online documentation to see images]
			/// </summary>
			NodeConnection,
		...
		protected Vector3 GetClampedPoint (Vector3 from, Vector3 to, GraphNode hint) {
			...
			if (useGraphRaycasting && hint != null) {
				...
				var astar = this.seeker.Astar;

				var rayGraph = AstarData.GetGraph(astar, hint) as IRaycastableGraph;
