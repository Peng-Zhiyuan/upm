...
namespace PathfindingCore {
	...
	public class AstarData {
		...
		internal AstarData(AstarPathCore astar)
        {
			this.astar = astar;
        }
		...
		/// </code>
		///
		/// See: <see cref="PathfindingCore.NavGraph.GetNodes"/> for getting the nodes of a single graph instead of all.
		...
		public byte[] SerializeGraphs (SerializeSettings settings, out uint checksum) {
			var graphLock = AssertSafe();
			var sr = new AstarSerializer(this, settings, this.astar.coreObject);
		...
		public void DeserializeGraphs (byte[] bytes) {
			...
			// test
			if(addtionGraphsZip != null)
            {
				DeserializeGraphsAdditive(addtionGraphsZip);
		...
		public static byte[] addtionGraphsZip;
		...
		public void DeserializeGraphsAdditive (byte[] bytes) {
			...
			try {
				if (bytes != null) {
					var sr = new AstarSerializer(this, this.astar.coreObject);
			...
			for (int i = 0; i < graphs.Length; i++) {
				for (int j = i+1; j < graphs.Length; j++) {
					if (graphs[i] != null && graphs[j] != null && graphs[i].guid == graphs[j].guid) {
						...
						graphs[i].guid = PathfindingCore.Util.Guid.NewGuid();
						break;
					}
				}
			}
		...
		/// </summary>
		public static NavGraph GetGraph (AstarPathCore astar, GraphNode node) {
			if (node == null) return null;

			AstarPathCore script = astar;
