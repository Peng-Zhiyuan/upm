...
namespace PathfindingCore.RVO {
	...
	///
	/// See: <see cref="PathfindingCore.AIBase.rvoDensityBehavior"/>
	/// </summary>
	[System.Serializable]
	public struct RVODestinationCrowdedBehavior {
		...
		public struct JobDensityCheck : PathfindingCore.Jobs.IJobParallelForBatched {
			[ReadOnly]
			RVOQuadtreeBurst quadtree;
			[ReadOnly]
			NativeArray<QueryData> data;
			[ReadOnly]
			NativeArray<float3> agentPosition;
			[ReadOnly]
			...
			void PathfindingCore.Jobs.IJobParallelForBatched.Execute (int start, int count) {
		...
		///
		/// See: <see cref="PathfindingCore.IAstarAI.destination"/>
		/// </summary>
		...
		public void Update (bool reachedDestination, ref bool isStopped, ref float rvoPriorityMultiplier, ref float rvoFlowFollowingStrength) {
			if (!enabled) {
				if (wasEnabled) {
