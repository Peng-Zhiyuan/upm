...
namespace PathfindingCore {
	...
	public class NodeLink3Node : PointNode {
		...
		public NodeLink3Node (AstarPathCore astar) {
	...
	public class NodeLink3 : GraphModifier {
		...
		FixedVector3 clamped1, clamped2;
		...
		public override void OnPostScan () {
			if (this.Astar.isScanning) {
				...
				this.Astar.AddWorkItem(new AstarWorkItem(force => {
					InternalOnPostScan();
					return true;
				}));
			}
		...
		public void InternalOnPostScan () {
			...
			if (this.Astar.data.pointGraph == null) {
				this.Astar.data.AddGraph(typeof(PointGraph));
			}

			//Get nearest nodes from the first point graph, assuming both start and end transforms are nodes
			startNode = this.Astar.data.pointGraph.AddNode(new NodeLink3Node(this.Astar), (Int3)StartTransform.position);
			startNode.link = this;
			endNode = this.Astar.data.pointGraph.AddNode(new NodeLink3Node(this.Astar), (Int3)EndTransform.position);
			endNode.link = this;
#else
...
#if !ASTAR_NO_POINT_GRAPH
			if (Application.isPlaying && this.Astar != null && this.Astar.data != null && this.Astar.data.pointGraph != null) {
				OnGraphsPostUpdate();
			}
#endif
		}
