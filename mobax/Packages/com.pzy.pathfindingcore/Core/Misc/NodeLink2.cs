...
namespace PathfindingCore {
	...
	public class NodeLink2 : GraphModifier {
		...
		public CoreTransform StartTransform {
			get { return Transform; }
		}

		public CoreTransform EndTransform {
			get { return end; }
		}
		...
		void CreateLinkNodes () {
			...
			if (this.Astar.data.graphs == null) return;

			if (this.Astar.data.pointGraph == null) {
				var graph = this.Astar.data.AddGraph(typeof(PointGraph)) as PointGraph;
			...
			if (startNode == null) {
				throw new System.Exception("[NodeLink2] not implement yet");
		...
		public override void OnCoreEnable()
		{
			...
			if (Application.isPlaying && this.Astar != null && this.Astar.data != null && !this.Astar.isScanning && isActiveAndEnabled && !linkCreated) {
				...
				this.Astar.AddWorkItem(context => context.SetGraphDirty(null));
			}
#endif
		}
		...
		public override void OnCoreDisable()
		{
			...
			if (this.Astar != null) {
				...
				if (startNode != null || endNode != null) {
					this.Astar.AddWorkItem(context => {
						if (startNode != null) {
		...
		/// The easiest is to do it inside a work item. See <see cref="AstarPathCore.active.AddWorkItem"/>.
		/// </summary>
		public void Apply (bool forceNewCheck) {
			...
			if (connectedNode1 == null || forceNewCheck) {
				var info = this.Astar.GetNearest(StartTransform.position, nn);
				connectedNode1 = info.node;
				clamped1 = info.position;
			}

			if (connectedNode2 == null || forceNewCheck) {
				var info = this.Astar.GetNearest(EndTransform.position, nn);
			...
			throw new System.Exception("[NodeLinke2] not complement yet;");
			// 以下四行代码一起
		...
		public override void DrawGizmos () {
			...
			if (StartTransform != null) {
				Draw.CircleXZ((Vector3)StartTransform.position, 0.4f, color);
			}
			if (EndTransform != null) {
				Draw.CircleXZ((Vector3)EndTransform.position, 0.4f, color);
			}
			...
			if (StartTransform != null && EndTransform != null) {
				...
				if (selected) {
					...
					NodeLink.DrawArch((Vector3)StartTransform.position+cross*0.1f, (Vector3)EndTransform.position+cross*0.1f, color);
					NodeLink.DrawArch((Vector3)StartTransform.position-cross*0.1f, (Vector3)EndTransform.position-cross*0.1f, color);
				}
			}
		}

		internal static void SerializeReferences (PathfindingCore.Serialization.GraphSerializationContext ctx) {
		...
		internal static void DeserializeReferences (PathfindingCore.Serialization.GraphSerializationContext ctx) {
