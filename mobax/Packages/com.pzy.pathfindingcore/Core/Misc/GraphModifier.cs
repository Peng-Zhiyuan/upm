...
namespace PathfindingCore {
	...
	public abstract class GraphModifier : VersionedMonoBehaviour {
		...
		public static void FindAllModifiers () {
			var allModifiers = CoreObject.FindObjectsOfType<GraphModifier>();

			for (int i = 0; i < allModifiers.Length; i++) {
				//if (allModifiers[i].enabled) allModifiers[i].OnEnable();
				if (allModifiers[i].IsEnabled) allModifiers[i].OnCoreEnable();
			}
		}
		...
		///
		/// Note: Area information (see <see cref="PathfindingCore.HierarchicalGraph)"/> may not be up to date when this event is sent.
		/// This means some methods like <see cref="PathfindingCore.PathUtilities.IsPathPossible"/> may return incorrect results.
		...
		///
		/// Area recalculations (see <see cref="PathfindingCore.HierarchicalGraph"/>) have been done at this stage so things like PathUtilities.IsPathPossible will work.
		///
