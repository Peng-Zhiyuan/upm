...
namespace PathfindingCore {
	...
	public class AnimationLink : NodeLink2 {
		...
		public void CalculateOffsets (List<Vector3> trace, out Vector3 endPosition) {
			...
			FixedVector3 prevOffset = Vector3.zero;
			FixedVector3 position = Transform.position;
			FixedVector3 firstOffset = Vector3.zero;
			...
			for (int i = 0; i < sequence.Length; i++) {
				...
				for (int repeat = 0; repeat < c.loopCount; repeat++) {
					...
					anim.Sample();
					FixedVector3 soffset = (FixedVector3)root.position - Transform.position;
					...
					for (int t = 0; t <= 20; t++) {
						...
						anim.Sample();
						FixedVector3 tmp = position + ((FixedVector3)root.position - Transform.position) + (FixedVector3)c.velocity * tf * c.clip.length;
						trace.Add(tmp);
					}
					position = position + (FixedVector3)c.velocity*1*c.clip.length;

					anim[c.clip.name].normalizedTime = 1;
					anim.Sample();
					FixedVector3 eoffset = (FixedVector3)root.position - Transform.position;
					prevOffset = eoffset;
				}
