	...
	public class AutoRepathPolicy {
		...
		public virtual bool ShouldRecalculatePath (IAstarAI ai) {
			...
			if (mode == Mode.EveryNSeconds) {
				...
				float squaredCost = (ai.destination - (FixedVector3)lastDestination).sqrMagnitude / Mathf.Max((ai.position - (FixedVector3)lastDestination).sqrMagnitude, ai.radius*ai.radius);
		...
		public void DrawGizmos (IAstarAI ai) {
			if (visualizeSensitivity && !float.IsPositiveInfinity(lastDestination.x)) {
				float r = Mathf.Sqrt(Mathf.Max(((Vector3)ai.position - lastDestination).sqrMagnitude, ai.radius*ai.radius)/(sensitivity*sensitivity));
				Draw.CircleXZ(lastDestination, r, Color.magenta);
			}
		}
	}
}
