...
namespace PathfindingCore {
	...
	public class NodeLink : GraphModifier {
		...
		public virtual void Apply () {
			if (Start == null || End == null || this.Astar == null) return;

			GraphNode startNode = this.Astar.GetNearest(Start.position).node;
			GraphNode endNode = this.Astar.GetNearest(End.position).node;
		...
		public static void LinkObjects () {
			...
			throw new System.Exception("[NodeLinker] not implement yet");
			//if (tfs.Length == 2) {
		...
		public static void DeleteLinks () {
			...
			for (int i = 0; i < tfs.Length; i++) {
				...
				{
					throw new System.Exception("[NodeLink] remove component not implement yet");
					//DestroyImmediate(conns[j]);
				}
			}
			SceneView.RepaintAll();
		}

		public static void LinkObjects (CoreTransform a, CoreTransform b, bool removeConnection) {
			...
			if (removeConnection) {
				if (connecting != null)
				{
					DestroyImmediate(connecting);
				}
			} else {
				if (connecting == null) {
					connecting = a.coreObject.AddComponent<NodeLink>();
					connecting.end = b;
				} else {
