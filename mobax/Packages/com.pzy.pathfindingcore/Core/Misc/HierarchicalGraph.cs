...
namespace PathfindingCore {
	...
	/// The connected component is exposed on nodes as the <see cref="PathfindingCore.GraphNode.Area"/> property and on this class using the <see cref="GetArea"/> method.
	///
	...
	/// [Open online documentation to see videos]
	///
	/// See: <see cref="PathfindingCore.PathUtilities.IsPathPossible"/>
	/// See: <see cref="PathfindingCore.NNConstraint"/>
	/// See: <see cref="PathfindingCore.GraphNode.Area"/>
	/// </summary>
	public class HierarchicalGraph {
		...
		public HierarchicalGraph (AstarPathCore astar) {
			this.astar = astar;
		...
		public JobHandle JobRecalculateIfNecessary (JobHandle dependsOn = default) {
			if (numDirtyNodes > 0) {
				lastJob = PathfindingCore.Jobs.IJobExtensions.ScheduleManaged(JobRecalculate, JobHandle.CombineDependencies(lastJob, dependsOn));
				return lastJob;
			} else {
				return dependsOn;
			}
		...
		public void RecalculateAll () {
			this.astar.data.GetNodes(AddDirtyNode);
		...
		public void OnDrawGizmos (DrawingData gizmos, RedrawScope redrawScope) {
			var hasher = new NodeHasher(this.astar);
