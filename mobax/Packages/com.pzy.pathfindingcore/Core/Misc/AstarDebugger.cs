...
namespace PathfindingCore {
	...
	public class AstarDebugger : VersionedMonoBehaviour {
		...
		public void Start () {
			//useGUILayout = false;
			Debug.Log("[AstarDebugger] useGUILayout property not support");
		...
		public void OnGUI () {
			...
			if (Time.realtimeSinceStartup - lastUpdate > 0.5f || cachedText == null || !Application.isPlaying) {
				...
				text.Append("A* Version: ").Append(AstarPathCore.Version.ToString());
				...
				if (showPathProfile) {
					AstarPathCore astar = this.Astar;
					...
					if (astar == null) {
						...
						if (PathfindingCore.Util.ListPool<Vector3>.GetSize() > maxVecPool) maxVecPool = PathfindingCore.Util.ListPool<Vector3>.GetSize();
						if (PathfindingCore.Util.ListPool<PathfindingCore.GraphNode>.GetSize() > maxNodePool) maxNodePool = PathfindingCore.Util.ListPool<PathfindingCore.GraphNode>.GetSize();
