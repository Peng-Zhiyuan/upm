...
/// It might be a bit hard to use directly, usually interfacing with the pathfinding system is done through the <see cref="PathfindingCore.Seeker"/> class.
///
/// \nosubgrouping
/// \ingroup relevant
/// </summary>
[HelpURL("http://arongranberg.com/astar/docs/class_astar_path.php")]
public class AstarPathCore : VersionedMonoBehaviour
{
    /// <summary>The version number for the A* %Pathfinding Project</summary>
    public static readonly System.Version Version = new System.Version(4, 3, 43);
    ...
    //// 需要在 Awake 之前被调用
    public void SetDefaultLoadGraphData(TextAsset data)
    {
        this.data = new AstarData(this);
        this.data.file_cachedStartup = data;
        this.data.cacheStartup = true;
    }

    /// <summary>
    /// 以追加的方式加载寻路图
    /// 相邻两个图之间的空间上重叠的节点会被创建连接
    /// 此方法会有额外开销，可预期的多次调用需要被合并
    // 
    /// 现在仅在类型为 RecastGraph 的图之间创建连接
    /// </summary>
    /// <param name="graphPackageDataList"></param>
    public void LoadGraphAdditional(List<byte[]> graphPackageDataList, List<FixedVector3> graphOffsetList, List<FixedVector3> graphRotationList)
    {
        var oldCount = this.data.graphs.Length;

        // 在数据管理器中加载 graph 包中的所有 graph
        for (int i = 0; i < graphPackageDataList.Count; i++)
        {
            var graphData = graphPackageDataList[i];
            var offset = graphOffsetList[i];
            var eulerAngles = graphRotationList[i];
            Quaternion rotation = Quaternion.Euler(eulerAngles.x, eulerAngles.y, eulerAngles.z);
            this.data.DeserializeGraphsAdditive(graphData);
            var graphList = this.data.graphs;
            var nowCount = graphList.Length;
            var newGraph = graphList[nowCount - 1];
            var recastGraphA = newGraph as RecastGraph;
            var m = Matrix4x4.TRS(offset, rotation, Vector3.one);
            recastGraphA.characterRadius = 0.2f;
            recastGraphA.rasterizeTerrain = false;
            recastGraphA.rasterizeTrees = false;
            recastGraphA.rasterizeColliders = false;
            recastGraphA.useTiles = true;
            recastGraphA.RelocateNodes(m);
        }

        // 拼接寻路图
        var graphsList = this.graphs;
        var modified = false;
        for (int i = oldCount - 1; i < graphsList.Length - 1; i++)
        {
            if (i == -1)
            {
                continue;
            ...
            var a = graphsList[i];
            var b = graphsList[i + 1];
            var recastGraphA = a as RecastGraph;
            var recastGraphB = b as RecastGraph;
            if (recastGraphA != null
                && recastGraphB != null)
            {
                var isChnaged = TryConnectAnyIntersectedNode(recastGraphA, recastGraphB);
                if (isChnaged)
                {
                    modified = true;
                }
    ...
    static bool TryConnectAnyIntersectedNode(RecastGraph g1, RecastGraph g2)
    {
        var modifiedAnyData = false;
        g1.showNodeConnections = true;
        g2.showNodeConnections = true;

        //// 获得两个图的三角节点列表
        //var g1NodeList = new List<TriangleMeshNode>();
        //g1.GetNodes(n =>
        //{
        //	g1NodeList.Add(n as TriangleMeshNode);
        //});

        //var g2NodeList = new List<TriangleMeshNode>();
        //g2.GetNodes(n =>
        //{
        //	g2NodeList.Add(n as TriangleMeshNode);
        //});
        g1.GetNodes(n1 =>
                        {
                            var tn1 = n1 as TriangleMeshNode;
                            g2.GetNodes(n2 =>
                                            {
                                                var tn2 = n2 as TriangleMeshNode;
                                                var (isOverlay, tn1SharedEdge) = IsTriangleMeshNodeIntersected(tn1, tn2);
                                                var (isOverlay2, tn2SharedEdge) = IsTriangleMeshNodeIntersected(tn2, tn1);
                                                if (isOverlay || isOverlay2)
                                                {
                                                    var p1 = tn1.position;
                                                    var p2 = tn2.position;
                                                    var cost = (p1 - p2).costMagnitude;
                                                    tn1.AddConnection(tn2, (uint)cost, tn1SharedEdge);
                                                    tn2.AddConnection(tn1, (uint)cost, tn2SharedEdge);
                                                    tn1.SetConnectivityDirty();
                                                    tn2.SetConnectivityDirty();
                                                    modifiedAnyData = true;
                                                }
                                            }
                            );
                        }
        );
        return modifiedAnyData;
    }

    // 判断两个三角面结点是否相交
    // 目前简单判断是否一个三角形内的任何顶点是否在另一个三角形内
    // 可能会出现判断不准确的情况
    // 返回1: 从 fromNode 到 toNode 是否临近
    // 返回2: 从 fromNode 到 toNode 的连接中的 fromNode 的共享边，如果不能连接返回 -1
    static (bool, int) IsTriangleMeshNodeIntersected(TriangleMeshNode fromNode, TriangleMeshNode toNode)
    {
        //Int3 n1v1;
        //Int3 n1v2;
        //Int3 n1v3;
        //n1.GetVertices(out n1v1, out n1v2, out n1v3);
        Int3 v0;
        Int3 v1;
        Int3 v2;
        fromNode.GetVertices(out v0, out v1, out v2);
        var count = 0;
        var overlayVetext0 = false;
        var overlayVetext1 = false;
        var overlayVetext2 = false;
        if (toNode.ContainsPoint(v0))
        {
            count++;
            overlayVetext0 = true;
        }
        if (toNode.ContainsPoint(v1))
        {
            count++;
            overlayVetext1 = true;
        }
        if (toNode.ContainsPoint(v2))
        {
            count++;
            overlayVetext2 = true;
        }
        ...
        // 当共享边使用 v0 和 v1 两个顶点时，使用 0 表示
        if (overlayVetext0 && overlayVetext1)
        {
            return (true, 0);
        }
        // 当共享边使用 v1 和 v2 两个顶点时，使用 1 表示
        if (overlayVetext1 && overlayVetext2)
        {
            return (true, 1);
        }
        else
        {
            // 当共享边使用 v2 和 v0 两个顶点时，使用 2 表示
            return (true, 2);
        }
    }

    static AstarPathCore _active;

    /// <summary>
    /// 最后创建的 AstarPathCore 对象
    /// 此静态字段仅在 Unity 平台可用
    /// </summary>
    public static AstarPathCore lastestInstance
    {
        get
        {
            ...
            {
                throw new Exception("[AstarPathCore] static field 'AstarPathCore.active' only avaliable in unity");
            }
        }
        set
        {
...
#region InspectorSettings
    ...
    /// See: <see cref="PathfindingCore.Heuristic"/> for more details and descriptions of the different modes.
...
#region Callbacks
    ...
    /// This is called at the start of the Awake call, right after <see cref="lastestInstance"/> has been set, but this is the only thing that has been done.\n
...
#region MemoryStructures
    ...
    /// See: <see cref="PathfindingCore.NavmeshCut"/>
    /// </summary>
    ...
    public AstarPathCore()
    {
        this.hierarchicalGraph = new HierarchicalGraph(this);
    ...
    /// This will set the <see cref="lastestInstance"/> property and deserialize all graphs.
    ///
    ...
    public static void FindAstarPath()
    {
        ...
        if (lastestInstance == null) lastestInstance = CoreObject.FindObjectOfType<AstarPathCore>();
        if (lastestInstance != null
            && (lastestInstance.data.graphs == null || lastestInstance.data.graphs.Length == 0)) lastestInstance.data.DeserializeGraphs();
    }
    ...
    /// See: AstarPath.GetTagNames
    /// </summary>
    public static string[] FindTagNames(AstarPathCore astar)
    {
        FindAstarPath();
        return astar != null ? astar.GetTagNames() : new string[1] { "There is no AstarPath component in the scene" };
    }
        ...
        for (int i = 0; i < graphs.Length; i++)
        {
                ...
                && graphs[i].drawGizmos)
            {
                graphs[i].GetNodes(node =>
                                {
                                    if (node.Walkable
                                        && (ignoreSearchTree || PathfindingCore.Util.GraphGizmoHelper.InSearchTree(node, debugPathData, debugPathID)))
                                    {
    ...
    public override void DrawGizmos()
    {
        ...
        if (lastestInstance == null) lastestInstance = this;
        if (lastestInstance != this
            || graphs == null)
        {
            return;
        }
    ...
    {
        if (logPathResults != PathLog.None
            && (path.error || logPathResults != PathLog.OnlyErrors))
        {
    ...
    private void PerformBlockingActions(bool force = false)
    {
        if (workItemLock.Held
            && pathProcessor.queue.AllReceiversBlocked)
        {
    ...
    /// </code>
    ///
    /// See: <see cref="PathfindingCore.IWorkItemContext"/>
    /// </summary>
...
#region GraphUpdateMethods
    ...
    public static int CalculateThreadCount(ThreadCount count)
    {
		...
		return 0;
#else
        if (count == ThreadCount.AutomaticLowLoad
            || count == ThreadCount.AutomaticHighLoad)
        {
#if ASTARDEBUG
    ...
    public override void OnCoreAwake()
    {
        ...
        if (CoreObject.FindObjectsOfType<AstarPathCore>().Length > 1)
        {
    ...
    void InitializePathProcessor()
    {
        ...
        pathProcessor.OnQueueUnblocked += () =>
        {
            ...
            {
                euclideanEmbedding.RecalculateCosts(this);
            }
        };
        if (multithreaded)
        {
    ...
    /// <summary>
    /// Internal method to make sure <see cref="lastestInstance"/> is set to this object and that <see cref="data"/> is not null.
    ...
    public override void OnCoreEnable()
    {
    ...
    public override void OnCoreDisable()
    {
        //if (active == this) {
        if (Application.isPlaying)
        {
    ...
    /// </summary>
    public override void OnCoreDestory()
    {
        if (logPathResults == PathLog.Heavy)
        ...
        var prevActive = lastestInstance;
        lastestInstance = this;
...
#region ScanMethods
    ...
    /// </code>
    ///
    /// Returns: A lock object. You need to call <see cref="PathfindingCore.PathProcessor.GraphUpdateLock.Release"/> on that object to allow pathfinding to resume.
    ...
    {
        throw new Exception("[AstarPathCore] Scan not supported in Runtime !");
    ...
    IEnumerable<Progress> ScanInternal(NavGraph[] graphsToScan, bool async)
    {
        ...
        euclideanEmbedding.dirty = true;
        euclideanEmbedding.RecalculatePivots(this);
        ...
        System.GC.Collect();
        if (logPathResults != PathLog.None
            && logPathResults != PathLog.OnlyErrors)
        {
    ...
    public static void BlockUntilCalculated(AstarPathCore active, Path path)
    {
        if (active == null)
            throw new System.Exception("Pathfinding is not correctly initialized in this scene (yet?). " + "AstarPath.active is null.\nDo not call this function in Awake");
        ...
        if (path.PipelineState < PathState.ReturnQueue)
        {
            ...
            else
            {
                while (path.PipelineState < PathState.ReturnQueue)
                {
                    if (active.pathProcessor.queue.IsEmpty
                        && path.PipelineState != PathState.Processing)
                    {
                        waitForPathDepth--;
    ...
    public static void RegisterSafeUpdate(AstarPathCore astar, System.Action callback)
    {
        astar.AddWorkItem(new AstarWorkItem(callback));
    }
    ...
    public void StartPath(Path path, bool pushToFront = false)
    {
        ...
        if (astar.graphs == null
            || astar.graphs.Length == 0)
        {
        ...
        // if (!Application.isPlaying)
        // {
        //     BlockUntilCalculated(this, path);
        // }
        BlockUntilCalculated(this, path);
    }
    ...
    public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
    {
        ...
        if (graphs != null)
        {
            for (int i = 0; i < graphs.Length; i++)
            {
                ...
                if (graph == null
                    || !constraint.SuitableGraph(i, graph))
                {
                    continue;
                }
        ...
        var a = !constraint.Suitable(nearestNode.node);
        if (!fullGetNearestSearch
        ...
        var b = !constraint.Suitable(nearestNode.node);
        var c = (nearestNode.clampedPosition - position).sqrMagnitude > maxNearestNodeDistanceSqr;
        if (b || (constraint.constrainDistance && c))
        {
            return new NNInfo();
        }
