...
namespace PathfindingCore {
	...
	public interface ITraversalProvider {
		...
		/// If this method returns true then a call to <see cref="CanTraverse(path,to)"/> must also return true.
	...
	public abstract class Path : IPathInternals {
		...
		/// See: <see cref="AstarPathCore.logPathResults"/>
		/// </summary>
		...
		/// See: WaitForPath
		/// </summary>
		public void BlockUntilCalculated (AstarPathCore astar) {
			AstarPathCore.BlockUntilCalculated(astar, this);
		}
		...
		internal uint CalculateHScore (GraphNode node) {
			...
			case Heuristic.Euclidean:
				...
				if (hTargetNode != null) {
					h = System.Math.Max(h, this.astar.euclideanEmbedding.GetHeuristic(node.NodeIndex, hTargetNode.NodeIndex));
				}
				return h;
		...
		/// Deprecated: Use the <see cref="PathfindingCore.Path.PipelineState"/> property instead
		/// </summary>
		...
		protected virtual void OnEnterPool () {
			if (vectorPath != null) PathfindingCore.Util.ListPool<Vector3>.Release(ref vectorPath);
			if (path != null) PathfindingCore.Util.ListPool<GraphNode>.Release(ref path);
		...
		protected virtual void Reset () {
			...
			if (System.Object.ReferenceEquals(this.astar, null))
			...
			path = PathfindingCore.Util.ListPool<GraphNode>.Claim();
			vectorPath = PathfindingCore.Util.ListPool<Vector3>.Claim();
			...
			heuristic = this.astar.heuristic;
			heuristicScale = this.astar.heuristicScale;

			enabledTags = -1;
			tagPenalties = null;

			pathID = this.astar.GetNextPathID();
		...
		protected void DebugStringSuffix (PathLog logMode, System.Text.StringBuilder text) {
			...
			if (logMode == PathLog.Heavy && !this.astar.IsUsingMultithreading) {
				text.Append("\nCallback references ");
