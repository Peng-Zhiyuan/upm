...
using PathfindingCore.WindowsStore;
...
namespace PathfindingCore.Serialization {
	...
	public class AstarSerializer {
		...
		/// </summary>
		private CoreObject contextRoot;
		...
		public AstarSerializer (AstarData data, CoreObject contextRoot) : this(data, SerializeSettings.Settings, contextRoot) {
		}

		public AstarSerializer (AstarData data, SerializeSettings settings, CoreObject contextRoot) {
			this.data = data;
		...
		byte[] SerializeMeta () {
			...
			meta.version = AstarPathCore.Version;
		...
		public bool OpenDeserialize (byte[] bytes) {
			...
			if (FullyDefinedVersion(meta.version) > FullyDefinedVersion(AstarPathCore.Version)) {
				Debug.LogWarning("Trying to load data from a newer version of the A* Pathfinding Project\nCurrent version: "+AstarPathCore.Version+" Data version: "+meta.version +
		...
		NavGraph DeserializeGraph (int zipIndex, int graphIndex, System.Type[] availableGraphTypes) {
			...
			if (ContainsEntry(jsonName)) {
				...
				//throw new Exception("[JsonSerializer] not implement yet");
				//TinyJsonDeserializer.Deserialize(GetString(GetEntry(jsonName)), graphType, graph, contextRoot);
				TinyJsonDeserializer.Deserialize(GetString(GetEntry(jsonName)), graphType, graph, null);
	...
	public class GraphMeta {
		...
		{
			var typeNameAtTargetIndex = typeNames[index];

			// The graph was null when saving. Ignore it
			if (String.IsNullOrEmpty(typeNameAtTargetIndex))
			{
				return null;
			}

			var targetNoNamespaceName = GetNoNamepaceTypeName(typeNameAtTargetIndex);

			for (int j = 0; j < availableGraphTypes.Length; j++) 
			{
				var noNamespaceName = availableGraphTypes[j].Name;

				//if (availableGraphTypes[j].FullName == typeNameAtTargetIndex)
				if(targetNoNamespaceName == noNamespaceName)
				{
					return availableGraphTypes[j];
				}
			}

			throw new Exception("No graph of type '" + typeNameAtTargetIndex + "' could be created, type does not exist");
		}

		/// <summary>
		/// 获得去掉命名空间的类型名称
		/// </summary>
		/// <param name="nameWithNamespace"></param>
		/// <returns></returns>
		static string GetNoNamepaceTypeName(string nameWithNamespace)
		{
			if (nameWithNamespace.Contains("."))
			{
				var parts = nameWithNamespace.Split('.');
