...
namespace PathfindingCore.Serialization {
	...
	public class TinyJsonSerializer {
		...
		TinyJsonSerializer () {
			...
			serializers[typeof(PathfindingCore.Util.Guid)] = v => output.AppendFormat("{{ \"value\": \"{0}\" }}", v.ToString());
	...
	public class TinyJsonDeserializer {
		...
		System.Object Deserialize (Type tp, System.Object populate = null) {
			...
			if (tpInfo.IsEnum) {
				...
				Eat("}");
				return result;
			} else if (Type.Equals(tp, typeof(PathfindingCore.Util.Guid))) {
				Eat("{");
				EatField();
				var result = PathfindingCore.Util.Guid.Parse(EatField());
				Eat("}");
				return result;
		...
		UnityEngine.Object DeserializeUnityObjectInner () {
			...
			var type = WindowsStoreCompatibility.GetTypeInfo(typeof(AstarPathCore)).Assembly.GetType(typename);
