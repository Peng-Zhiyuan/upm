...
namespace PathfindingCore {
	...
	/// Alternatively you can use the <see cref="PathfindingCore.FunnelModifier Funnel"/> which works better on navmesh/recast graphs or the <see cref="PathfindingCore.RaycastModifier"/>.
	///
	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI {
		...
		public bool reachedDestination {
			get {
				...
				if (remainingDistance + ((Vector3)dir).magnitude >= 0.05f) return false;

				return true;
			}
		}

		public FixedVector3 destination { get; set; }
		...
		///
		/// Deprecated: In 4.0 this will automatically add a <see cref="PathfindingCore.AIDestinationSetter"/> component and set the target on that component.
		...
		public Transform target {
			...
			set {
				...
				var setter = GetComponent<AIDestinationSetter>();
				if (setter == null) setter = coreObject.AddComponent<AIDestinationSetter>();
				setter.target = value;
		...
		Vector3 IAstarAI.velocity {
			get {
				return Time.deltaTime > 0.00001f ? (previousPosition1 - previousPosition2) / (Fixed)Time.deltaTime : (FixedVector3)Vector3.zero;
			}
		}
		...
		Vector3 IAstarAI.steeringTarget {
			get {
				...
				return interpolator.valid ? (FixedVector3)interpolator.position + (FixedVector3)interpolator.tangent : simulatedPosition;
			}
		}
		...
		FixedVector3 previousPosition1, previousPosition2, simulatedPosition;
		FixedQuaternion simulatedRotation;
		...
		public virtual void SearchPath () {
			...
			SetPath(ABPath.Construct(this.Astar, currentPosition, destination, null));
		}
