...
namespace PathfindingCore {
	...
	public class Seeker : VersionedMonoBehaviour {
		...
		///
		/// If you know the name of the graph you can use the <see cref="PathfindingCore.GraphMask.FromGraphName"/> method:
		/// <code>
   ...
   //         {
		...
		public ABPath GetNewPath (Vector3 start, Vector3 end) {
			...
			return ABPath.Construct(this.Astar, start, end, null);
		}
		...
		public Path StartPath (Vector3 start, Vector3 end, OnPathDelegate callback) {
			return StartPath(ABPath.Construct(this.Astar, start, end, null), callback);
		}
		...
		public Path StartPath (AstarPathCore astar, Vector3 start, Vector3 end, OnPathDelegate callback, GraphMask graphMask) {
			return StartPath(ABPath.Construct(astar, start, end, null), callback, graphMask);
		}
		...
		void StartPathInternal (Path p, OnPathDelegate callback) {
			...
			// AstarPathCore.StartPath(path);
			var astar = this.coreEngine.FindStuff<AstarPathCore>();
			astar.StartPath(path);
		}
		...
		public MultiTargetPath StartMultiTargetPath (Vector3 start, Vector3[] endPoints, bool pathsForAll, OnPathDelegate callback = null, int graphMask = -1) {
			MultiTargetPath p = MultiTargetPath.Construct(this.Astar, start, endPoints, null, null);
		...
		public MultiTargetPath StartMultiTargetPath (Vector3[] startPoints, Vector3 end, bool pathsForAll, OnPathDelegate callback = null, int graphMask = -1) {
			MultiTargetPath p = MultiTargetPath.Construct(this.Astar, startPoints, end, null, null);
