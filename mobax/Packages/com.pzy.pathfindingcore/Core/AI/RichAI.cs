...
namespace PathfindingCore {
	...
	public partial class RichAI : AIBase, IAstarAI {
		...
		///
		/// See: <see cref="PathfindingCore.FunnelModifier"/>
		/// </summary>
		...
		public override bool reachedDestination {
			get {
				...
				if (distanceToSteeringTarget + movementPlane.ToPlane(steeringTarget - richPath.Endpoint).magnitude + movementPlane.ToPlane(destination - (FixedVector3)richPath.Endpoint).magnitude > endReachedDistance) return false;
		...
		public override Vector3 endOfPath {
			get {
				return hasPath ? richPath.Endpoint : (Vector3)destination;
			}
		}
		...
		public override void Teleport (Vector3 newPosition, bool clearPath = true) {
			...
			var nearest = this.Astar != null? this.Astar.GetNearest(newPosition) : new NNInfo();
			float elevation;
