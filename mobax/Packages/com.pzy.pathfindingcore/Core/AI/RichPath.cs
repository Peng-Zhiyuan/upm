...
namespace PathfindingCore {
	public class RichPath {
		...
		/// scene called 'Moving'.
		///
		/// See: <see cref="PathfindingCore.Examples.LocalSpaceRichAI"/>
		/// </summary>
		...
		public void Initialize (Seeker seeker, Path path, bool mergePartEndpoints, bool simplificationMode) {
			...
			for (int i = 0; i < nodes.Count; i++) {
				if (nodes[i] is TriangleMeshNode) {
					...
					var astar = this.seeker.coreEngine.FindStuff<AstarPathCore>();
					var graph = AstarData.GetGraph(astar, nodes[i]) as NavmeshBase;
		...
		/// See: <see cref="PathfindingCore.IAstarAI.GetRemainingPath"/>
		/// </summary>
	...
	public abstract class RichPathPart : PathfindingCore.Util.IAstarPooledObject {
	...
	public class RichFunnel : RichPathPart {
		...
		public RichFunnel () {
			left = PathfindingCore.Util.ListPool<Vector3>.Claim();
			right = PathfindingCore.Util.ListPool<Vector3>.Claim();
	...
	public class RichSpecial : RichPathPart {
		...
		public CoreTransform first;
		public CoreTransform second;
		public bool reverse;
