...
namespace PathfindingCore {
	...
	/// If you want to make the AI to follow a particular object you can attach the <see cref="PathfindingCore.AIDestinationSetter"/> component.
	...
	///
	/// Here is a video of this script being used move an agent around (technically it uses the <see cref="PathfindingCore.Examples.MineBotAI"/> script that inherits from this one but adds a bit of animation support for the example scenes):
	...
	/// The <see cref="SearchPath"/> method will prepare a path request and send it to the <see cref="PathfindingCore.Seeker"/> component which should be attached to the same GameObject as this script.
	/// Since this script will when waking up register to the <see cref="PathfindingCore.Seeker.pathCallback"/> delegate this script will be notified every time a new path is calculated by the <see cref="OnPathComplete"/> method being called.
	...
	public partial class CustomAIPath : AIBase, IAstarAI {
		/// <summary>
		...
		/// <tr><td>[Open online documentation to see videos]</td><td>\xmlonly <verbatim><span class="label label-warning">Ok</span><br/></verbatim>\endxmlonly A low value but a high acceleration works decently to make the AI follow the path more closely. Note that the <see cref="PathfindingCore.AILerp"/> component is better suited if you want the agent to follow the path without any deviations.</td></tr>
		...
		/// When this option is enabled a <see cref="AstarPathCore.GetNearest"/> query will be done every frame to find the closest node that the agent can walk on
		...
		/// If you are using a navmesh/recast graph you may want to switch to the <see cref="PathfindingCore.RichAI"/> movement script which is specifically written for navmesh/recast graphs and
		...
		protected virtual void UpdateMovementPlane () {
			...
			var graph = AstarData.GetGraph(astar, path.path[0]) as ITransformedGraph;
		...
		protected override void MovementUpdateInternal (float deltaTime, out Vector3 nextPosition, out Quaternion nextRotation) {
			/*tr.forward = this.destination - tr.position;
			var dir = deltaTime * maxSpeed * tr.forward;
			nextPosition = tr.position + deltaTime * maxSpeed * tr.forward;
			nextRotation = tr.rotation;
			...
			bool stopped = isStopped || (reachedDestination && whenCloseToDestination == CloseToDestinationMode.Stop);*/
				...
				if (interpolator.valid && !stopped) {
					...
					velocity2D += dir.normalized * maxSpeed;
				...
				if (rvoController != null && rvoController.IsEnabled) {
		...
		protected Vector2 rotationFilterState, rotationFilterState2;
		...
		protected override Vector3 ClampToNavmesh (Vector3 position, out bool positionChanged) {
			if (constrainInsideGraph) {
				...
				var clampedPosition = this.Astar.GetNearest(position, cachedNNConstraint).position;
