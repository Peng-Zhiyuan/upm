...
namespace PathfindingCore {
	...
	/// See: <see cref="PathfindingCore.IAstarAI"/> (all movement scripts implement this interface)
	/// </summary>
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		public AstarPathCore Astar
        {
			get
            {
				var ret = this.coreEngine.FindStuff<AstarPathCore>();
				return ret;
            }
        }
		...
		///
		/// See: <see cref="PathfindingCore.RVO.RVODestinationCrowdedBehavior"/>
		...
		{ 
			get 
			{ 
				return updatePosition ? tr.position : (FixedVector3)simulatedPosition; 
			} 
		}
		...
		public Quaternion rotation {
			get { return updateRotation ? tr.rotation : (FixedQuaternion)simulatedRotation; }
			set {
				if (updateRotation) {
					tr.rotation = value;
				} else {
		...
		///
		/// Deprecated: In 4.1 this will automatically add a <see cref="PathfindingCore.AIDestinationSetter"/> component and set the target on that component.
		...
		public Transform target {
			...
			set {
				...
				if (!TryGetComponent(out AIDestinationSetter setter)) setter = coreObject.AddComponent<AIDestinationSetter>();
				setter.target = value;
		...
		static void OnUpdate (AIBase[] components, int count, BatchedEvents.Event ev) {
			...
			if (RVOSimulator.active != null) {
				...
				for (int i = 0; i < count; i++) agentsWithRVOControllers += (components[i].rvoController != null && components[i].rvoController.IsEnabled ? 1 : 0);
				...
				for (int i = 0, j = 0; i < count; i++) {
					...
					if (agent.rvoController != null && agent.rvoController.IsEnabled) {
		...
		public virtual void SearchPath () {
			...
			ABPath p = ABPath.Construct(this.Astar, start, end, null);
			SetPath(p);
		}
		...
		public override void DrawGizmos () {
			if (!Application.isPlaying || !IsEnabled) FindComponents();

			var color = ShapeGizmoColor;
			if (rvoController != null && rvoController.locked) color *= 0.5f;
			if (orientation == OrientationMode.YAxisForward) {
				Draw.WireCylinder((Vector3)position, Vector3.forward, 0, radius * tr.localScale.x, color);
			} else {
				Draw.WireCylinder((Vector3)position, rotation * Vector3.up, tr.localScale.y * height, radius * tr.localScale.x, color);
			}

			if (!float.IsPositiveInfinity(destination.x) && Application.isPlaying) Draw.CircleXZ((Vector3)destination, 0.2f, Color.blue);
