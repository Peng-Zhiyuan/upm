...
namespace PathfindingCore {
	...
	public class GraphUpdateScene : GraphModifier {
		...
		public Bounds GetBounds () {
			...
			{
				throw new System.Exception("[GraphUpdateScene] 此处使用了 transform 数, CoreEngine 并不支持");
		...
		public void Apply () {
			if (this.Astar == null) {
				...
				Debug.LogError("There is no AstarPath object in the scene", coreObject.debbugerGameObject as UnityEngine.Object);
				return;
			}
			...
			if (points == null || points.Length == 0) {
				...
				if (polygonCollider != null) {
					...
					throw new System.Exception("[GraphUpdateScene] transform motrix not support");
					...
					if (bounds.center == Vector3.zero && bounds.size == Vector3.zero) {
						Debug.LogError("Cannot apply GraphUpdateScene, no points defined and no renderer or collider attached", coreObject.debbugerGameObject as UnityEngine.Object);
						return;
					}
