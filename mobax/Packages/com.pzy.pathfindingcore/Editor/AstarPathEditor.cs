...
namespace PathfindingCore {
	[CustomCoreComponentInspector(typeof(AstarPathCore))]
	public class AstarPathEditor : CoreComponentInspector {
		...
		public AstarPathCore script { get; private set; }
		...
		public override void OnEnable () {
			script = target as AstarPathCore;
		...
		void DrawAboutArea () {
			...
			if (FullyDefinedVersion(AstarUpdateChecker.latestVersion) > FullyDefinedVersion(AstarPathCore.Version) || FullyDefinedVersion(AstarUpdateChecker.latestBetaVersion) > FullyDefinedVersion(AstarPathCore.Version)) {
				if (FullyDefinedVersion(AstarUpdateChecker.latestVersion) <= FullyDefinedVersion(AstarPathCore.Version)) {
			...
			if (FullyDefinedVersion(newVersion) > FullyDefinedVersion(AstarPathCore.Version)) {
		...
		void DrawSerializationSettings () {
			...
			if (serializationSettingsArea.BeginFade()) {
				...
				if (GUILayout.Button("Generate cache")) {
					var serializationSettings = new PathfindingCore.Serialization.SerializeSettings();
				...
				if (GUILayout.Button("Save to file")) {
					...
					if (path != "") {
						var serializationSettings = PathfindingCore.Serialization.SerializeSettings.Settings;
						...
						PathfindingCore.Serialization.AstarSerializer.SaveToFile(path, bytes);
				...
				if (GUILayout.Button("Load from file")) {
					...
					if (path != "") {
						try {
							byte[] bytes = PathfindingCore.Serialization.AstarSerializer.LoadFromFile(path);
		...
		void DrawPathfindingSettings () {
			...
			int threads = AstarPathCore.CalculateThreadCount(script.threadCount);
		...
		public static void EditTags () {
			...
			AstarPathCore astar = CoreObject.FindObjectOfType<AstarPathCore>();

			if (astar != null) {
				editTags = true;
				showSettings = true;
				//Selection.activeGameObject = astar.gameObject;
				Selection.activeGameObject = astar.coreObject.debbugerGameObject as GameObject;
			} else {
		...
		void CheckGraphEditors (bool forceRebuild = false) {
			if (forceRebuild || graphEditors == null || script.graphs == null || script.graphs.Length != graphEditors.Length) {
				...
				for (int i = 0; i < script.graphs.Length; i++) {
					...
					if (graph.guid == new PathfindingCore.Util.Guid()) {
						graph.guid = PathfindingCore.Util.Guid.NewGuid();
					}
				...
				for (int i = 0; i < script.graphs.Length; i++) {
					...
					if (script.graphs[i].guid == new PathfindingCore.Util.Guid()) {
						script.graphs[i].guid = PathfindingCore.Util.Guid.NewGuid();
					}
		...
		public byte[] SerializeGraphs (out uint checksum) {
			var settings = PathfindingCore.Serialization.SerializeSettings.Settings;

			settings.editorSettings = true;
			return SerializeGraphs(settings, out checksum);
		}

		public byte[] SerializeGraphs (PathfindingCore.Serialization.SerializeSettings settings, out uint checksum) {
			byte[] bytes = null;
			uint tmpChecksum = 0;
			...
			for (int i = 0; i < graphEditors.Length; i++) {
				...
				output.Length = 0;
				PathfindingCore.Serialization.TinyJsonSerializer.Serialize(graphEditors[i], output);
			...
			AstarPathCore.lastestInstance.FlushWorkItems();
		...
		void DeserializeGraphs (byte[] bytes) {
			try {
				...
				for (int i = 0; i < graphEditors.Length; i++) {
					...
					if (data != null)
					{
						//Pathfinding.Serialization.TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(), graphEditors[i], script.gameObject);

						// 没有 GameObject
						PathfindingCore.Serialization.TinyJsonDeserializer.Deserialize(data, graphEditors[i].GetType(), graphEditors[i], null);
					}
				}
			} catch (System.Exception e) {
		...
		public static void MenuScan () {
			if (AstarPathCore.lastestInstance == null) {
				//AstarPath.active = FindObjectOfType<AstarPath>();
				AstarPathCore.lastestInstance = CoreObject.FindObjectOfType<AstarPathCore>();
				if (AstarPathCore.lastestInstance == null) {
					return;
				}
			}

			if (!Application.isPlaying && (AstarPathCore.lastestInstance.data.graphs == null || AstarPathCore.lastestInstance.data.graphTypes == null)) {
				EditorUtility.DisplayProgressBar("Scanning", "Deserializing", 0);
				AstarPathCore.lastestInstance.data.DeserializeGraphs();
			}

			try {
				var lastMessageTime = Time.realtimeSinceStartup;
				foreach (var p in AstarPathCore.lastestInstance.ScanAsync()) {
