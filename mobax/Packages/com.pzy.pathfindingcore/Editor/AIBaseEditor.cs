...
namespace PathfindingCore {
	...
	public class BaseAIEditor : EditorBase {
		...
		protected override void Inspector () {
			var isAIPath = typeof(AIPath).IsAssignableFrom(target.GetType()) || typeof(CustomAIPath).IsAssignableFrom(target.GetType());
			...
			var mono = target as CoreComponent;
			mono.TryGetComponent<PathfindingCore.RVO.RVOController>(out PathfindingCore.RVO.RVOController rvoController);
			if (rvoController && rvoController.IsEnabled) {
			...
			if (isRichAI && Application.isPlaying && AstarPathCore.lastestInstance != null && AstarPathCore.lastestInstance.graphs.Length > 0 && AstarPathCore.lastestInstance.data.recastGraph == null && AstarPathCore.lastestInstance.data.navmesh == null) {
