	...
	public static class AstarUpdateChecker {
		...
		public static System.Version latestVersion {
			get {
				...
				return _latestVersion ?? AstarPathCore.Version;
			}
			private set {
		...
		public static System.Version latestBetaVersion {
			get {
				RefreshServerMessage();
				return _latestBetaVersion ?? AstarPathCore.Version;
			}
			private set {
		...
		static bool CheckForUpdates () {
			...
			var offsetMinutes = (Application.isPlaying && Time.time > 60) || AstarPathCore.lastestInstance != null ? -20 : 20;
		...
		static void DownloadVersionInfo () {
			...
			var script = AstarPathCore.lastestInstance != null ? AstarPathCore.lastestInstance : CoreObject.FindObjectOfType<AstarPathCore>();
			...
			string query = updateURL+
						   "?v="+AstarPathCore.Version+
						   "&pro=1"+
						   "&check="+updateCheckRate+"&distr="+AstarPathCore.Distribution+
		...
		static void ShowUpdateWindowIfRelevant () {
			...
			var skipVersion = new System.Version(EditorPrefs.GetString("AstarSkipUpToVersion", AstarPathCore.Version.ToString()));

			if (AstarPathEditor.FullyDefinedVersion(latestVersion) != AstarPathEditor.FullyDefinedVersion(skipVersion) && AstarPathEditor.FullyDefinedVersion(latestVersion) > AstarPathEditor.FullyDefinedVersion(AstarPathCore.Version)) {
