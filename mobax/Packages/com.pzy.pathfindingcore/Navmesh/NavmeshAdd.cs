...
namespace PathfindingCore {
	...
	public class NavmeshAdd : NavmeshClipper {
		...
		/// </summary>
		public override bool RequiresUpdate (PathfindingCore.Util.GridLookup<NavmeshClipper>.Root previousState) {
			return (tr.position - (FixedVector3)previousState.previousPosition).sqrMagnitude > updateDistance*updateDistance || (useRotationAndScale && (Quaternion.Angle(previousState.previousRotation, tr.rotation) > updateRotationDistance));
		}
		...
		/// See: <see cref="PathfindingCore.NavmeshUpdates.updateInterval"/>
		/// See: <see cref="PathfindingCore.NavmeshUpdates.ForceUpdate()"/>
		/// </summary>
		public override void ForceUpdate () {
			this.Astar.navmeshUpdates.ForceUpdateAround(this);
		}
		...
		internal override void NotifyUpdated (PathfindingCore.Util.GridLookup<NavmeshClipper>.Root previousState) {
		...
		public Vector3 Center {
			get {
				return tr.position + (useRotationAndScale ? tr.TransformPoint(center) : (FixedVector3)center);
			}
		}
		...
		/// </summary>
		public override Rect GetBounds (PathfindingCore.Util.GraphTransform inverseTransform) {
			if (this.verts == null) RebuildMesh();
			var verts = PathfindingCore.Util.ArrayPool<Int3>.Claim(this.verts != null? this.verts.Length : 0);
			int[] tris;
			...
			PathfindingCore.Util.ArrayPool<Int3>.Release(ref verts);
			return r;
		}
		...
		public void GetMesh (ref Int3[] vbuffer, out int[] tbuffer, PathfindingCore.Util.GraphTransform inverseTransform = null) {
			...
			if (useRotationAndScale) {
				Matrix4x4 m = Matrix4x4.TRS(tr.position + (FixedVector3)center, tr.rotation, tr.localScale * meshScale);
				...
				Vector3 voffset = tr.position + (FixedVector3)center;
				for (int i = 0; i < verts.Length; i++) {
