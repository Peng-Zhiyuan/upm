...
namespace PathfindingCore {
	...
	public class NavmeshCut : NavmeshClipper {
			...
			for (int i = 0; i < verts.Length; i++) {
				if (pointers.ContainsKey(i)) {
					...
					do {
						...
						if (s == -1) {
							Debug.LogError("Invalid Mesh '"  + mesh.name + " in " + coreObject.Name);
							break;
						}
					} while (s != i);
		...
		internal float GetY (GraphTransform transform) {
			return transform.InverseTransform(useRotationAndScale ? tr.TransformPoint(center) : tr.position + (FixedVector3)center).y;
		}
		...
		public override void DrawGizmos () {
			...
			var graph = this.Astar != null ? (this.Astar.data.recastGraph as NavmeshBase ?? this.Astar.data.navmesh) : null;
