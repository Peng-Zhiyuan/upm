...
namespace PathfindingCore.Util {
	...
	public struct NodeHasher {
		...
		public NodeHasher(AstarPathCore active) {
			hasher = default;
	...
	public class GraphGizmoHelper : IAstarPooledObject, System.IDisposable {
		...
		readonly System.Action<GraphNode, GraphNode> drawConnection;
		...
		public static GraphGizmoHelper GetSingleFrameGizmoHelper (DrawingData gizmos, AstarPathCore active, RedrawScope redrawScope) {
			return GetGizmoHelper(gizmos, active, DrawingData.Hasher.NotSupplied, redrawScope);
		}

		public static GraphGizmoHelper GetGizmoHelper (DrawingData gizmos, AstarPathCore active, DrawingData.Hasher hasher, RedrawScope redrawScope) {
		...
		public void Init (AstarPathCore active, DrawingData.Hasher hasher, DrawingData gizmos, RedrawScope redrawScope) {
			if (active != null) {
		...
		public void DrawConnections (GraphNode node) {
			if (showSearchTree) {
				...
				node.GetConnections(other => drawConnection(node, other));
			}
		}

		// pzy:
		// 绘制连接
		void DrawConnection (GraphNode thisNode, GraphNode other)
		{
			//var color = drawConnectionColor;
			var color = Color.gray;
			color.a = 0.7f;
			
			builder.Line(drawConnectionStart, ((Vector3)other.position + drawConnectionStart)*0.5f, color);

			//// pzy:
			//// 绘制 cost
			//if(other is MeshNode)
   //         {
			//	uint cost = 0;
			//	var meshNode = other as MeshNode;
			//	foreach (var c in meshNode.connections)
   //             {
			//		if(c.node == thisNode)
   //                 {
			//			cost = c.cost;
			...
			//		}
   //             }
			//	var a = (Vector3)thisNode.position;
			//	var b = (Vector3)other.position;
			//	var mid = (a + b) / 2;
			//	builder.Label2D(mid, cost.ToString(), 10, LabelAlignment.Center);
			//}
