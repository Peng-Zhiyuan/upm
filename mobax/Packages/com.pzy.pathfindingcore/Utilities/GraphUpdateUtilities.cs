...
namespace PathfindingCore {
	...
	/// This class works a lot with the GraphNode class, a useful function to get nodes is <see cref="AstarPathCore.GetNearest"/>.
	///
	/// See: <see cref="AstarPathCore.GetNearest"/>
	/// See: <see cref="PathfindingCore.PathUtilities"/>
	///
	/// \since Added in 3.1
	///
	/// \ingroup utils
	...
	public static class GraphUpdateUtilities {
		...
		public static bool UpdateGraphsNoBlock (AstarPathCore astar, GraphUpdateObject guo, GraphNode node1, GraphNode node2, bool alwaysRevert = false) {
			List<GraphNode> buffer = ListPool<GraphNode>.Claim();

			buffer.Add(node1);
			buffer.Add(node2);

			bool worked = UpdateGraphsNoBlock(astar, guo, buffer, alwaysRevert);
		...
		/// <param name="alwaysRevert">If true, reverts the graphs to the old state even if no blocking occurred</param>
		public static bool UpdateGraphsNoBlock (AstarPathCore astar, GraphUpdateObject guo, List<GraphNode> nodes, bool alwaysRevert = false) {
			bool worked;

			// Pause pathfinding while modifying the graphs
			var graphLock = astar.PausePathfinding();
