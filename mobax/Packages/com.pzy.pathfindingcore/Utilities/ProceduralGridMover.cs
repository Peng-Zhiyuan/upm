...
namespace PathfindingCore {
	...
	public class ProceduralGridMover : VersionedMonoBehaviour {
		...
		void Start () {
			if (this.Astar == null) throw new System.Exception("There is no AstarPath object in the scene");
			...
			if (graph == null) {
				...
				if (graphIndex >= this.Astar.data.graphs.Length) throw new System.Exception("The ProceduralGridMover was configured to use graph index " + graphIndex + ", but only " + this.Astar.data.graphs.Length + " graphs exist");

				graph = this.Astar.data.graphs[graphIndex] as GridGraph;
		...
		public void UpdateGraph () {
			...
			this.Astar.AddWorkItem(new AstarWorkItem(
				(context, force) => {
				...
				try {
					...
					Debug.LogException(e, this.coreObject.debbugerGameObject as UnityEngine.Object);
					done = true;
				}
