...
namespace PathfindingCore {
	...
	/// You can use this with recast graphs as well. However since recast graph updates are much slower it is recommended to use the <see cref="PathfindingCore.NavmeshCut"/> component if at all possible.
	///
	...
	public class DynamicGridObstacle : GraphModifier {
		...
		public override void OnCoreAwake () {
			...
			if (coll == null && coll2D == null && Application.isPlaying) {
				throw new System.Exception("A collider or 2D collider must be attached to the GameObject(" + coreObject.Name + ") for the DynamicGridObstacle to work");
			}
		...
		public override void OnCoreUpdate()
		{
			...
			if (coll == null && coll2D == null) {
				Debug.LogError("No collider attached to this GameObject. The DynamicGridObstacle component requires a collider.", this.coreObject.debbugerGameObject as UnityEngine.Object);
				//enabled = false;
				IsEnabled = false;
				return;
			}
			...
			if (this.Astar == null || this.Astar.isScanning || Time.realtimeSinceStartup - lastCheckTime < checkTime || !Application.isPlaying || pendingGraphUpdates.Count > 0) {
				return;
			}
		...
		public void DoUpdateGraphs () {
			...
			if (!colliderEnabled) {
				...
				if (BoundsVolume(merged) < BoundsVolume(newBounds) + BoundsVolume(prevBounds)) {
					...
					this.Astar.UpdateGraphs(guo1);
					this.Astar.UpdateGraphs(guo2);
				}
