...
namespace PathfindingCore {
	...
	/// This class works a lot with the <see cref="PathfindingCore.GraphNode"/> class, a useful function to get nodes is AstarPath.GetNearest.
	/// See: <see cref="AstarPathCore.GetNearest"/>
	/// See: <see cref="PathfindingCore.GraphUpdateUtilities"/>
	/// See: <see cref="PathfindingCore.GraphUtilities"/>
	/// \ingroup utils
	/// </summary>
	public static class PathUtilities {
		...
		public static void GetPointsAroundPointWorldFlexible (AstarPathCore astar, Vector3 center, Quaternion rotation, List<Vector3> positions) {
			if (positions.Count == 0) return;

			var snapped = astar.GetNearest(center, NNConstraint.Default);
			...
			for (int k = 0; k < iterations; k++) {
				...
				for (int i = 0; i < positions.Count; i++) {
					...
					var near = astar.GetNearest(p, nn).position;
				...
				groupPos = astar.GetNearest(newCenter, nn).position;
			}
