 ...
 *   the default type returned by JsonMapper.Instance.ToObject().
 *
    ...
    public class JsonData : IJsonWrapper, IEquatable<JsonData>
    {
        ...
        object IDictionary.this[object key] {
            ...
            set {
                if (! (key is String))
                {
                    throw new ArgumentException($"The key has to be a string (key: {key}, type: {key?.GetType()})");
                }
		...
		public T TryGet<T>(string key, T @default = default)
        {
            ...
            var jd = dic[key] as JsonData;
            if (typeof(T) == typeof(string))
            {
				return (T)System.Convert.ChangeType(jd.ToString(),typeof(string));
                //return jd.ToString();
            }
            ...
            {
                return (T)System.Convert.ChangeType(long.Parse(jd.ToString()), typeof(long));
            }
            else if (typeof(T) == typeof(bool))
            {
                ...
                if (str == "1")
                {
					return (T)System.Convert.ChangeType(true,typeof(bool));
                }
                else if(str == "0")
                {
					return (T)System.Convert.ChangeType(false,typeof(bool));
                }
				return (T)System.Convert.ChangeType(bool.Parse(jd.ToString()),typeof(bool));
            }
            else if (typeof(T) == typeof(JsonData))
            {
				return (T)System.Convert.ChangeType(jd,typeof(JsonData));
            }
			return  (T)System.Convert.ChangeType(jd,typeof(object));
        }
        ...
        public long ToLong()
        {
            ...
            if(jw.IsInt)
            {
                var intValue = jw.GetInt();
                return (long)intValue;
            }
            else if(jw.IsLong)
            {
                var longValue = jw.GetLong();
                return longValue;
            }
            else if(jw.IsString)
            {
                var stringValue = jw.GetString();
                ...
                {
                    var longValue = long.Parse(stringValue);
                    return longValue;
                }
            }
            else
            {
                var jsonStorageType = jw.GetJsonType();
                throw new Exception($"[JsonData] ToLong() error, jsonStorageType: {jsonStorageType}");
            }
        }
