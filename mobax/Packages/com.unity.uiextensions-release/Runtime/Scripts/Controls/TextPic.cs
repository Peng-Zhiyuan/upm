...
namespace UnityEngine.UI.Extensions {
    ...
    public class TextPic : Text, IPointerClickHandler, IPointerExitHandler, IPointerEnterHandler, ISelectHandler {
		...
		{
			// pzy:
			// hrefName 可能包含引号，在编辑器中可以正常打开
			// 但是在安卓上不可以
			hrefName = hrefName.Trim('"');
			Debug.Log("[TextPic] trimed:" + hrefName);
        ...
        public void OnPointerClick(PointerEventData eventData) { 
			...
			//Debug.Log($"[TextPic] lp: {lp}");
			//Debug.Log($"[TextPic] m_HrefInfos.Count: {m_HrefInfos.Count}");
			for (int h = 0; h < m_HrefInfos.Count; h++) 
			{
				//Debug.Log($"[TextPic] m_HrefInfos[{h}].boxes.Count: {m_HrefInfos[h].boxes.Count}");
				for (int i = 0; i < m_HrefInfos[h].boxes.Count; ++i) 
				{
					//Debug.Log($"[TextPic] m_HrefInfos[{h}].boxes[{i}]: {m_HrefInfos[h].boxes[i]}");
					if (m_HrefInfos[h].boxes[i].Contains(lp)) 
					{
						//Debug.Log($"[TextPic] contains: {m_HrefInfos[h].name}");
