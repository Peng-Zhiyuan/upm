...
namespace BehaviorDesigner.Runtime
{
    public abstract class SharedVariable
    {
        ...
        [SerializeField]
        private CoreObject mPropertyMappingOwner;
        public CoreObject PropertyMappingOwner { get { return mPropertyMappingOwner; } set { mPropertyMappingOwner = value; } }
    ...
    public abstract class SharedVariable<T> : SharedVariable
    {
        ...
        public override void InitializePropertyMapping(BehaviorSource behaviorSource)
        {
            ...
            if (!string.IsNullOrEmpty(PropertyMapping)) {
                ...
                CoreObject gameObject = null;
                try {
                    if (!Equals(PropertyMappingOwner, null)) {
                        gameObject = PropertyMappingOwner;
                    } else {
                        gameObject = behaviorSource.OwnerAsComponent.coreObject;
                    }
                } catch (Exception /*e*/) {
                    var behavior = behaviorSource.OwnerAsComponent;
