...
namespace BehaviorDesigner.Runtime
{
    ...
    public class BehaviorManager : CoreComponent
    {
        static private BehaviorManager _latestInstace;
        static public BehaviorManager latestInstance
        {
            get
            {
                ...
                {
                    return _latestInstace;
                }
                else
                {
                    throw new Exception($"[BehaviorManager] static field '{nameof(latestInstance)}' only supported in Unity");
                }
            }
            set
            {
        ...
        //private UpdateIntervalType updateInterval = UpdateIntervalType.Manual;
        ...
        {
            //Debug.Log("[BehaviorManager] EnableBehavior: " + behavior);
            // 已经创建过内部数据
            ...
            else if (pausedBehaviorTrees.TryGetValue(behavior, out behaviorTree)) 
            { // unpause
            ...
            {
                throw new Exception("[BehaviorManager] async loader not implement yet");
            ...
            {
                behaviorTree = LoadBehavior(behavior, behavior.coreObject, behavior.Transform, behavior.coreObject.Name);
        ...
        private class BehaviorThreadLoader
        {
            ...
            private CoreObject gameObject;
            private CoreTransform transform;
            private string gameObjectName;
            private Func<Behavior, CoreObject, CoreTransform, string, BehaviorTree> loadBehaviorAction;
            ...
            public BehaviorThreadLoader(Behavior behaviorTree, Func<Behavior, CoreObject, CoreTransform, string, BehaviorTree> action)
            {
                behavior = behaviorTree;
                gameObject = behavior.coreObject;
            ...
            {
                behaviorTree = loadBehaviorAction(behavior, gameObject, transform, gameObjectName);
            }
        }

        // 异步数据加载模式会用到
        // 创建一个协程，用于判断异步数据加载队列是否全部完成
        // 如果队列已完成，则结束携程
        ...
        private BehaviorTree LoadBehavior(Behavior behavior, CoreObject behaviorGameObject, CoreTransform behaviorTransform, string gameObjectName)
        {
        ...
        private int AddToTaskList(BehaviorTree behaviorTree, Task task, CoreObject behaviorGameObject, CoreTransform behaviorTransform, ref bool hasExternalBehavior, TaskAddData data)
        {
            if (task == null) {
                return -3;
            }
        ...
        public override void OnCoreLateUpdate()
        {
        ...
        public void Tick()
        {
            //Debug.Log("[BehaviorManager] behaviorTrees.Count: " + behaviorTrees.Count);
            for (int i = 0; i < behaviorTrees.Count; ++i) 
            {
            ...
            for (int i = 0; i < behaviorTree.conditionalReevaluate.Count; ++i) {
                ...
                if (conditionalStatus != behaviorTree.conditionalReevaluate[i].taskStatus) {
                    ...
                    if (behaviorTree.behavior.LogTaskChanges) {
                        ...
                        UnityEngine.Debug.Log(string.Format("{0}: {1}: Conditional abort with task {2} ({3}, index {4}) because of conditional task {5} ({6}, index {7}) with status {8}",
        ...
        private TaskStatus RunTask(BehaviorTree behaviorTree, int taskIndex, int stackIndex, TaskStatus previousStatus)
        {
            ...
            if (task.Disabled) {
                if (behaviorTree.behavior.LogTaskChanges) {
                    UnityEngine.Debug.Log(string.Format("{0}: {1}: Skip task {2} ({3}, index {4}) at stack index {5} (task disabled)", RoundedTime(), behaviorTree.behavior.ToString(),
        ...
        private void PushTask(BehaviorTree behaviorTree, int taskIndex, int stackIndex)
        {
            ...
            if (activeStack.Count == 0 || activeStack.Peek() != taskIndex) {
                ...
                if (behaviorTree.behavior.LogTaskChanges) {
                    UnityEngine.Debug.Log(string.Format("{0}: {1}: Push task {2} ({3}, index {4}) at stack index {5}", RoundedTime(), behaviorTree.behavior.ToString(),
        ...
        private void PopTask(BehaviorTree behaviorTree, int taskIndex, int stackIndex, ref TaskStatus status, bool popChildren, bool notifyOnEmptyStack)
        {
            ...
            if (behaviorTree.behavior.LogTaskChanges) {
                UnityEngine.Debug.Log(string.Format("{0}: {1}: Pop task {2} ({3}, index {4}) at stack index {5} with status {6}", RoundedTime(), behaviorTree.behavior.ToString(),
