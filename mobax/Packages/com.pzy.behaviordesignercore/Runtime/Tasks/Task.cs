...
namespace BehaviorDesigner.Runtime.Tasks
{
    ...
    public abstract class Task
    {
        ...
        // Called when the graph is drawing the text near the node.
        public virtual string OnDrawNodeText() { return string.Empty; }

        // pzy: core engine not support coroutine
        ...
        public CoreObject GameObject { set { gameObject = value; } }
        protected CoreObject gameObject;
        public CoreTransform Transform { set { transform = value; } }
        protected CoreTransform transform;

        protected T GetComponent<T>() where T : CoreComponent
        {
            return gameObject.GetComponent<T>();
        }

        protected CoreComponent GetComponent(System.Type type)
        {
            return gameObject.GetComponent(type);
        }

        protected void TryGetComponent<T>(out T component) where T : CoreComponent
        {
            gameObject.TryGetComponent<T>(out component);
        }

        protected void TryGetComponent(System.Type type, out CoreComponent component)
        {
        ...
        protected CoreObject GetDefaultGameObject(CoreObject go)
        {
            if (go == null) {
                return gameObject;
            }
