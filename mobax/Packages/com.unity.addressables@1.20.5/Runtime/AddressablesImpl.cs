...
namespace UnityEngine.AddressableAssets
{
    ...
    public class AddressableInject
    {
        public static List<DownloadEntry> downloadEntry = new List<DownloadEntry>();
    }
    ...
    internal class AddressablesImpl : IEqualityComparer<IResourceLocation>
    {
        ...
        public AsyncOperationHandle<long> GetDownloadSizeAsync(IEnumerable keys)
        {
            ...
            foreach (object key in keys)
            {
                ...
                foreach (var loc in locations)
                {
                    if (loc.HasDependencies)
                    {
                        ...
                        // pzy:
                        // 调试用代码
                        //var dpList = loc.Dependencies;
                        //foreach (var dp in dpList)
                        //{
                        //    if (dp.InternalId.Contains("$env_slum_itema_tex_asset"))
                        //    {
                        //        Debug.Log($"[{loc.Dependencies.Count}]  {loc.InternalId} -> {dp.InternalId}");
                        //    }
                        //}
            ...
            AddressableInject.downloadEntry.Clear();
            ...
            foreach (IResourceLocation location in allLocations.Distinct())
            {
                ...
                if (sizeData != null)
                {
                    var thisSize = sizeData.ComputeSize(location, ResourceManager);
                    size += thisSize;
                    if(thisSize > 0)
                    {
                        var entry = new DownloadEntry();
                        entry.name = location.PrimaryKey;
                        entry.size = thisSize;
                        AddressableInject.downloadEntry.Add(entry);
                    }
                }
