...
public class GameObjectPool
{
    ...
    public void Remove(GameObject keyObj)
    {
        if (dic.ContainsKey(keyObj))
        {
            var goQueue = dic[keyObj];
            Debug.Log($"[gameobject pool] pool: {keyObj.name} released " + goQueue.Count);
    ...
    public T Reuse<T>(GameObject prefab, bool isActive = true, Type defaultType = null, Transform parent = null) where T : RecycledGameObject
    {
        if (prefab == null)
        {
        ...
        if (recycleableGo == null)
        {
            ...
            recycleableGo.bucketName = bucketName;
