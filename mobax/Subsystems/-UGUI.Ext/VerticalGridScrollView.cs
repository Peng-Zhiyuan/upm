...
public class VerticalGridScrollView : MonoBehaviour
{
	...
	private float ContentTop
	{
		...
		{
			return 0;
			//return this.content.offsetMin.y;
		}
	}
	...
	private float ContentLeft
	{
		...
		{
			return 0;
			//return this.content.offsetMin.x;
		}
	}
	...
	private float ContentRight
	{
		get
		{
			...
			//return this.content.offsetMax.x;
		}
	}
    ...
    private int rowCount
	{
		get
		{
			return Mathf.Max(1, Mathf.FloorToInt((this.content.rect.width - offsetX) / this.cellWidth));
		}
    }
	...
	private int LeftPadding
	{
		get
		{
			return Mathf.FloorToInt((this.content.rect.width - offsetX - (rowCount * cellWidth)))/2;
		}
	}
	...
	{
        var delta = this.ViewBottom - this.ContentBottom;
	...
	{
		Debug.LogError("this.content.offsetMin:"+this.content.offsetMin+ "  this.content.offsetMax:"+this.content.offsetMax);
        var headTop = this.head.localPosition.y + this.cellHeight / 2;
        var topDistance = headTop - this.ViewTop;
        var indexY = Mathf.FloorToInt(topDistance / this.cellHeight);
        var index = indexY * this.rowCount;
		Debug.LogError("index:"+index+" this.rowCount:"+this.rowCount);
	...
	private void PlaceControl() 
	{
		...
		Debug.LogError("PlaceControl:"+this.dataList.Count);
		int index;
		float y;
        this.CalculateFirstActiveControlIndex(out index, out y);
		Debug.LogError("index:"+index+"  y:"+y);
        if (index < 0) index = 0;
        if (!this.IsIndexValidate(index)) 
		{
            return;
	...
	public void ChangeData<T>(IList<T> dataList, VirtaulGridScrollViewOption option = VirtaulGridScrollViewOption.None)
	{
		...
		Debug.LogError("this.dataList:"+this.dataList.Count);
		this.Refresh(option);
	}
