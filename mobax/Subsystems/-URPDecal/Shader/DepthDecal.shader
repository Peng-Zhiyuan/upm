Shader "Universal Render Pipeline/DepthDecal"
{
    Properties
    {
        [Header(Basic)]
        ...
        [Enum(UnityEngine.Rendering.BlendMode)]_SrcBlend("_SrcBlend", Float) = 5 // 5 = SrcAlpha
        [Enum(UnityEngine.Rendering.BlendMode)]_DstBlend("_DstBlend", Float) = 10 // 10 = OneMinusSrcAlpha

        [Header(Alpha remap)]//(extra alpha control)
        _AlphaRemap("_AlphaRemap", vector) = (1,0,0,0)// (default = 1,0,0,0) _____alpha will first mul x, then add y    (zw unused)

        [Header(Prevent Side Stretching)]//(Compare projection direction with scene normal and Discard if needed)
        [Toggle(_ProjectionAngleDiscardEnable)] _ProjectionAngleDiscardEnable("_ProjectionAngleDiscardEnable", float) = 0
        _ProjectionAngleDiscardThreshold("_ProjectionAngleDiscard", range(-1,1)) = 0
        ...
        [Enum(UnityEngine.Rendering.CompareFunction)]_StencilComp("_StencilComp", Float) = 0 // (default = Disable) _____Set to NotEqual if you want to mask by specific _StencilRef value, else set to Disable
        ...
        [Enum(UnityEngine.Rendering.CompareFunction)]_ZTest("_ZTest", Float) = 0 // (default = Disable) _____to improve GPU performance, Set to LessEqual if camera never goes into cube volume, else set to Disable
        ...
        [Enum(UnityEngine.Rendering.CullMode)]_Cull("_Cull", Float) = 1 // (default = Front) _____to improve GPU performance, Set to Back if camera never goes into cube volume, else set to Front

        //[Header(Unity Fog)]
        //[Toggle(_UnityFogEnable)] _UnityFogEnable("_UnityFogEnable", Float) = 1// (default = on)

        [Header(Support Orthographic camera)]
        [Toggle(_SupportOrthographicCamera)] _SupportOrthographicCamera("_SupportOrthographicCamera", Float) = 0// (default = off)
    }
    ...
    SubShader
    {
        ...
        Tags { "LightMode" = "GroundDecal" }
            ...
            #pragma shader_feature_local DEPTH_DECAL
            //#define DEPTH_DECAL
