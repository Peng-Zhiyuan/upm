Shader "NGRole/Liquid"
{
	...
	{		
		...
		//液体部分
		_LiquidHeight("Liquid Height",float)=0.5
		_SectionColor ("Section Color", Color) = (1,1,1,1)
		_WaterColor ("Water Color", Color) = (1,1,1,1)
		_WaterColor2("Water Color2", Color) = (1,1,1,1)
		_WaterGradualRate("Water Gradual Rate",Range(0,1)) = 0.5 
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 _WaterColor;
			half4 _WaterColor2;
			half _WaterGradualRate;
			half4 _FoamColor;
			half4 _ForceDir;
			half _WaveHeight;
			half4 _CenterPosition;
			half _HeightScale;
			half _TargetHeight;

			half GetWaveHeight(half3 worldPos){
				half3 disVector = half3(worldPos.x, _CenterPosition.y + _LiquidHeight* _HeightScale, worldPos.z) - half3(0, _CenterPosition.y + _LiquidHeight * _HeightScale, 0);
				half dis = length(disVector);
				half d = dot(disVector, _ForceDir.xyz);
				return _LiquidHeight + dis * d * _WaveHeight;
			}
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				if(i.worldPos.y - (_CenterPosition.y + _TargetHeight * _HeightScale) >  0.001)
				{
				  	discard;
				}
				...
				half fd = dot(viewDirection, i.worldNormal);
				...
				if(i.worldPos.y > (_CenterPosition.y + (_TargetHeight * _HeightScale - _FoamHeight)))
				{
					return  _FoamColor;
                }
				return lerp(_WaterColor, _WaterColor2, saturate((i.worldPos.y-_CenterPosition.y) / _HeightScale * _WaterGradualRate));
			}
			ENDHLSL
		}
	...
	/*	Pass {
			...
			#pragma shader_feature_local _USE_REFLECTION_MAP
			#pragma shader_feature_local _USE_SHADOW_COLOR_TEXTURE
			#pragma shader_feature_local _USE_RAMP_TEXTURE
			#pragma shader_feature_local _USE_HSV_SHADOW
			#pragma shader_feature_local _USE_NORMAL_SHADOW
		...
		Pass{
			Name "Forward_Teleport"
			Tags{"LightMode" = "Cloth_Teleport"}
			Blend SrcAlpha OneMinusSrcAlpha
			Cull[_Cull]
			ZWrite On
			HLSLPROGRAM
		...
		Pass
		{
			...
			#pragma fragmentoption ARB_precision_hint_fastest       // 最快速精度
			#pragma shader_feature_local _OUTLINE_UV2_AS_NORMALS
			#pragma shader_feature_local _OUTLINE_USE_VERTEXCOLOR
