...
public static class NG_Utils
{
	...
	public static string ToShader (this TextureChannel channel)
	{
		switch(channel)
		{
			...
			default: Debug.LogError("[NG_Utils] Unrecognized texture channel: " + channel.ToShader()); return null;
		}
	}
	...
	public static TextureChannel FromShader(string str)
	{
		...
		switch(str)
		{
			...
			default: Debug.LogError("[NG_Utils] Unrecognized texture channel from shader: " + str + "\nDefaulting to Alpha"); return TextureChannel.Alpha;
		}
	}
	...
	public static bool ShaderKeywordRadio(string header, string[] keywords, GUIContent[] labels, List<string> list, ref bool update)
	{
		...
		if(hasHeader)
		{
			...
			NG_GUI.HelpButton(helpRect, helpTopic);
	...
	{
		var paths = Directory.GetFiles(Application.dataPath, "!NG Readme.txt", SearchOption.AllDirectories);
		if(paths.Length > 0)
		{
	...
	public static Mesh CreateSmoothedMesh(Mesh originalMesh, string format, bool vcolors, bool vtangents, bool uv2, bool overwriteMesh)
	{
		...
		{
			Debug.LogWarning("[NG : Smoothed Mesh] Supplied OriginalMesh is null!\nCan't create smooth normals version.");
			return null;
		}
		...
		for (int i = 0; i < newMesh.vertexCount; i++)
		{
			Vector3 bitangent = Vector3.Normalize(Vector3.Cross(newMesh.normals[i], new Vector3(newMesh.tangents[i].x, newMesh.tangents[i].y, newMesh.tangents[i].z)) * newMesh.tangents[i].w);
			Matrix4x4 btn = new Matrix4x4(newMesh.tangents[i], new Vector4(bitangent.x, bitangent.y, bitangent.z, 1), new Vector4(newMesh.normals[i].x, newMesh.normals[i].y, newMesh.normals[i].z, 1), Vector4.one);
			Matrix4x4 mat = btn.transpose;
			//Debug.Log(averageNormals[i]);
			averageNormals[i] = Vector3.Normalize(mat.MultiplyVector(averageNormals[i]));
			//averageNormals[i] = Vector3.Normalize(btn.MultiplyVector(averageNormals[i]));
			//Debug.Log(averageNormals[i]);
		}
	...
	public static PackedFile PackFile(string windowsPath)
	{
		if(!File.Exists(windowsPath))
		{
			...
			Debug.LogError("[NG PackFile] File doesn't exist:" + windowsPath);
			return null;
		}
		...
		{
			EditorApplication.Beep();
			Debug.LogError("[NG PackFile] Can't find NG Readme file!\nCan't determine root folder to pack/unpack files.");
			return null;
		}
	...
	public static void CreateArchive(PackedFile[] packedFiles, string outputFile)
	{
		...
		{
			EditorApplication.Beep();
			Debug.LogError("[NG PackFile] No file to pack!");
			return;
		}
		...
		sbIndex.AppendLine("# NG PACKED SHADERS");
		var cursor = 0;
		...
		Debug.Log("[NG CreateArchive] Created archive:\n" + fullPath);
	}
	...
	public static PackedFile[] ExtractArchive(string archivePath, string filter = null)
	{
		...
		if(archiveLines[0] != "# NG PACKED SHADERS")
		{
			EditorApplication.Beep();
			Debug.LogError("[NG ExtractArchive] Invalid NG archive:\n" + archivePath);
			return null;
		}
		...
		for(var line = 1; line < archiveLines.Length; line++)
		{
			...
			{
				EditorApplication.Beep();
				Debug.LogError("[NG ExtractArchive] Invalid format in NG archive, at line " + line + ":\n" + archivePath);
				return null;
			}
