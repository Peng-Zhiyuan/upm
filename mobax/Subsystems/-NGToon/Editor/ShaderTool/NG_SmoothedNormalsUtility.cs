...
public class NG_SmoothedNormalsUtility : EditorWindow
{
	[MenuItem(NG_Menu.MENU_PATH + "Smoothed Normals Utility", false, 500)]
	static void OpenTool()
	{
		GetWindowNG();
	}

	private static NG_SmoothedNormalsUtility GetWindowNG()
	{
		var window = GetWindow<NG_SmoothedNormalsUtility>(true, "NG : Smoothed Normals Utility", true);
	...
	private Mesh CreateSmoothedMeshAsset(SelectedMesh originalMesh)
	{
		...
		var rootPath = mCustomDirectory ? Application.dataPath + "/" + mCustomDirectoryPath + "/" : NG_Utils.FindReadmePath() + OUTPUT_FOLDER;
		...
		var newAssetName = originalMeshName + "_" + MESH_SUFFIX + ".asset";
