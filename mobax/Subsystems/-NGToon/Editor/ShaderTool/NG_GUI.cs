...
public static class NG_GUI
{
	...
	private static Texture2D GetCustomTexture(string name)
	{
		...
		var rootPath = NG_Utils.FindReadmePath(true);
	...
	private static GUIStyle EnabledLabel
	{
		get
		{
			if(_EnabledLabel == null)
			{
				...
				_EnabledLabel.normal.background = GetCustomTexture("NG_EnabledBg");
			}
	...
	public static GUIStyle ShurikenFoldout
	{
		get
		{
			if(_ShurikenFoldout == null)
			{
				...
				_ShurikenFoldout.normal.background = GetCustomTexture("NG_FoldoutArrowRight");
				...
				_ShurikenFoldout.onNormal.background = GetCustomTexture("NG_FoldoutArrowDown");
	...
	public static GUIStyle ContextualHelpBox
	{
		get
		{
			if(_ContextualHelpBox == null)
			{
				...
				_ContextualHelpBox.normal.background = GetCustomTexture("NG_ContextualHelpBox");
	...
	public static GUIStyle ContextualHelpBoxHover
	{
		get
		{
			if(_ContextualHelpBoxHover == null)
			{
				...
				_ContextualHelpBoxHover.hover.background = GetCustomTexture("NG_ContextualHelpBoxHover");
	...
	public static GUIStyle EnabledPropertyHelpBoxExp
	{
		get
		{
			if(_EnabledPropertyHelpBoxExp == null)
			{
				...
				_EnabledPropertyHelpBoxExp.normal.background = GetCustomTexture("NG_EnabledBgPropertyExpanded");
	...
	public static GUIStyle EnabledPropertyHelpBox
	{
		get
		{
			if(_EnabledPropertyHelpBox == null)
			{
				...
				_EnabledPropertyHelpBox.normal.background = GetCustomTexture("NG_EnabledBgProperty");
			}
	...
	public static GUIStyle ErrorPropertyHelpBoxExp
	{
		get
		{
			if(_ErrorPropertyHelpBoxExp == null)
			{
				...
				_ErrorPropertyHelpBoxExp.normal.background = GetCustomTexture("NG_ErrorBgPropertyExpanded");
	...
	public static GUIStyle ErrorPropertyHelpBox
	{
		get
		{
			if(_ErrorPropertyHelpBox == null)
			{
				...
				_ErrorPropertyHelpBox.normal.background = GetCustomTexture("NG_ErrorBgProperty");
			}
	...
	public static GUIStyle Tab
	{
		get
		{
			if(_Tab == null)
			{
				...
				_Tab.normal.background = GetCustomTexture("NG_TabOff");
				_Tab.focused.background = GetCustomTexture("NG_TabOff");
				_Tab.active.background = GetCustomTexture("NG_TabOff");

				_Tab.onNormal.background = GetCustomTexture("NG_Tab");
				_Tab.onFocused.background = GetCustomTexture("NG_Tab");
				_Tab.onActive.background = GetCustomTexture("NG_Tab");
	...
	public static GUIStyle HelpIcon
	{
		get
		{
			if(_HelpIcon == null)
			{
				...
				//_HelpIcon.normal.background = GetCustomTexture("NG_HelpIcon");
				//_HelpIcon.active.background = GetCustomTexture("NG_HelpIcon_Down");
			}
			...
			if(_HelpIcon2 == null)
			{
				...
				//_HelpIcon2.normal.background = GetCustomTexture("NG_HelpIcon2");
				//_HelpIcon2.active.background = GetCustomTexture("NG_HelpIcon2_Down");
			}
	...
	public static GUIStyle CogIcon
	{
		get
		{
			if(_CogIcon == null)
			{
				...
				_CogIcon.normal.background = GetCustomTexture("NG_CogIcon");
				_CogIcon.active.background = GetCustomTexture("NG_CogIcon_Down");
			}
	...
	public static Texture2D GetHelpBoxIcon(MessageType msgType)
	{
		switch(msgType)
		{
			case MessageType.Error:
				return GetCustomTexture("NG_ErrorIcon");
			case MessageType.Warning:
				return GetCustomTexture("NG_WarningIcon");
			case MessageType.Info:
				return GetCustomTexture("NG_InfoIcon");
		}
	...
	{
		EditorGUILayout.LabelField(GUIContent.none, new GUIContent(message, GetCustomTexture("NG_ContextIcon")), canHover ? ContextualHelpBoxHover : ContextualHelpBox);
	}
...
public class NGHeaderHelpDecorator : MaterialPropertyDrawer
{
	protected readonly string header;
	protected readonly string help;

	public NGHeaderHelpDecorator(string header)
	{
		this.header = header;
		help = null;
	}
	public NGHeaderHelpDecorator(string header, string help)
	{
		this.header = header;
		this.help = help;
	}
...
public class NGHelpBoxDecorator : MaterialPropertyDrawer
{
	...
	public NGHelpBoxDecorator(string messageType, string msg)
	{
		msgType = (MessageType)Enum.Parse(typeof(MessageType), messageType);
		message = ParseMessage(msg);
		icon = NG_GUI.GetHelpBoxIcon(msgType);
	}

	public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
	{
		position.height -= 4f;
		EditorGUI.LabelField(position, GUIContent.none, new GUIContent(message, icon), NG_GUI.HelpBoxRichTextStyle);
	...
	public override float GetPropertyHeight(MaterialProperty prop, string label, MaterialEditor editor)
	{
		...
		var height = NG_GUI.HelpBoxRichTextStyle.CalcHeight(new GUIContent(message, icon), Screen.width - (scrollBar ? 51 : 34));
		return height + 6f;
	}
}
...
public class NGSeparatorDecorator : MaterialPropertyDrawer
{
	public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
	{
		position.y += 4;
		position.height -= 4;
		NG_GUI.Separator(position);
	}
...
public class NGOutlineNormalsGUIDrawer : MaterialPropertyDrawer
{
	...
	readonly string[] keywords = { "NG_NONE", "NG_COLORS_AS_NORMALS", "NG_TANGENT_AS_NORMALS", "NG_UV2_AS_NORMALS" };

	public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
	{
		NG_GUI.Header("OUTLINE NORMALS", "Defines where to take the vertex normals from to draw the outline.\nChange this when using a smoothed mesh to fill the gaps shown in hard-edged meshes.");

		var r = EditorGUILayout.GetControlRect();
		r = EditorGUI.IndentedRect(r);
		var index = GetCurrentIndex(prop);
		EditorGUI.BeginChangeCheck();
		index = NG_GUI.RadioChoiceHorizontal(r, index, labels);
...
public class NGVector4FloatsDrawer : MaterialPropertyDrawer
{
	...
	public NGVector4FloatsDrawer(string groupLabel, string labelX, string labelY, string labelZ, string labelW)
	{
	...
	public NGVector4FloatsDrawer(string groupLabel, string labelX, string labelY, string labelZ, string labelW, float minX, float maxX, float minY, float maxY, float minZ, float maxZ, float minW, float maxW)
	{
		this.groupLabel = groupLabel;
		labels = new[] { labelX, labelY, labelZ, labelW };
...
public class NGColorNoAlphaDrawer : MaterialPropertyDrawer
{
...
public class NGGradientDrawer : MaterialPropertyDrawer
{
	...
	public override void OnGUI(Rect position, MaterialProperty prop, string label, MaterialEditor editor)
	{
		...
		{
			var lastSavePath = NG_GradientManager.LAST_SAVE_PATH;
			if(!lastSavePath.Contains(Application.dataPath))
				lastSavePath = Application.dataPath;

			var path = EditorUtility.SaveFilePanel("Create New Ramp Texture", lastSavePath, "NG_CustomRamp", "png");
			if(!string.IsNullOrEmpty(path))
			{
				...
				NG_GradientManager.CreateAndSaveNewGradientTexture(256, projectPath);
				...
				//Open for editing
				NG_RampGenerator.OpenForEditing(texture, editor.targets, true);
			}
		}
		...
		{
			NG_RampGenerator.OpenForEditing((Texture2D)prop.textureValue, editor.targets, true);
		}
		GUI.enabled = enabled;
	...
	Texture2D TryFindDefaultRampTexture()
	{
		...
		{
			var defaultTexPath = "Assets" + rootPath + "/Textures/NG_Ramp_3Levels.png";
...
public class NGHeaderDecorator : MaterialPropertyDrawer
{
	protected readonly string header;

	public NGHeaderDecorator(string header)
	{
		this.header = header;
	}
...
public class NGKeywordFilterDrawer : MaterialPropertyDrawer
{
	protected readonly string[] keywords;

	public NGKeywordFilterDrawer(string keyword)
	{
...
internal class NGToggleNoKeywordDrawer : MaterialPropertyDrawer
{
