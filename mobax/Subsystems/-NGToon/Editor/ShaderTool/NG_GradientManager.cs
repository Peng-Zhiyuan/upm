...
public class NG_GradientManager
{
	public static string LAST_SAVE_PATH
	{
		get { return EditorPrefs.GetString("NG_GradientSavePath", Application.dataPath); }
		set { EditorPrefs.SetString("NG_GradientSavePath", value); }
	}
	...
	public static void SetGradientFromUserData(string userData, Gradient gradient)
	{
		...
		if(keys == null || keys.Length < 3 || keys[0] != "GRADIENT")
		{
			...
			Debug.LogError("[NG_GradientManager] Invalid Gradient Texture\nMake sure the texture was created with the Ramp Generator.");
			return;
		}
