...
Shader "Universal Render Pipeline/NGStylized Lit"
{
    Properties
    {
...
*/
        ...
        _ShadowThreshold ("Shadow Threshold", Range(0,1)) = 0.75
        ...
        [Toggle(HEIGHT_MAP)] _UseHeightMap("Use Height Map", Float) = 0.0
        _HeightMap("Height Map", 2D) = "white" {}
        _HeightScale("Height Scale",  Range(0.0, 1)) = 0.003

        _OcclusionStrength("Strength", Range(0.0, 1.0)) = 1.0
        //_OcclusionMap("Occlusion", 2D) = "white" {}

        [ToggleOff(_EMISSION)] _Emission("Emission", Float) = 1.0
        ...
        //CubeMap
        //[Toggle(CUBE_MAP)] _UseCubeMap("Use CubeMap", float) = 0
       
        [KeywordEnum(NONE_MAP, CUBE_MAP, MAT_CAP)] _UseReflectMap("Use Reflect Map", Int) = 0
        _ReflectAmount("Reflect Amount", Range(0, 1)) = 1.0
       // _ReflectMask("Reflect Mask", 2D) = "white" {}
        _MapColor("Map Color", Color) = (1, 1, 1)
        _CubeMap("Environment Map",CUBE) = "_Skybox"{}
        _MatMap("MatMap", 2D) = "white" {}
       

        //Vertex fog
        //[Toggle(HEIGHT_FOG)] _OpenHeightFog("Height Fog", float) = 0
		...
		_FogMin("Height Fog Min", Float) = -5
		_FogMax("Height Fog Max", Float) = 5
		...
		//[Header(STANDARD fog properties overrides)]
		 ...
		 [Toggle(FOG_ANIM)]_ANIMATION("Use fog animation", Float) = 0
         ...
         [HideInInspector]_GlobalShadowColor("Global Shadow Color", Color) = (0, 0, 0, 0)
         [HideInInspector]_GlobalSunColor("Global Sun Color", Color) = (0, 0, 0, 0)
         [HideInInspector]_GlobalSunPos("Global Sun Pos", Vector) = (0, 0, 0, 0)
    }
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #pragma multi_compile _ _ADDITIONAL_LIGHTS //_ADDITIONAL_LIGHTS_VERTEX //NEED
            //#pragma multi_compile _ _ADDITIONAL_LIGHT_SHADOWS//
            #pragma multi_compile _ _SHADOWS_SOFT
  
            #pragma multi_compile _MIXED_LIGHTING_SUBTRACTIVE
            ...
            #pragma multi_compile  _ LIGHTMAP_ON//NEED
            #pragma multi_compile  _ LIGHTMAP_SHADOW_MIXING//NEED
            #pragma multi_compile  _ SHADOWS_SHADOWMASK//NEED
            //#pragma multi_compile_fog
            //#pragma multi_compile _ HEIGHT_FOG
            #pragma multi_compile _ HEIGHT_MAP CUBE_MAP //MAT_CAP
            //--------------------------------------
            // GPU Instancing
            ...
            #include "NGStylizedLitInput.hlsl"
            #include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Lighting.hlsl"
            #include "NGParallax.hlsl"
            #include "NGHeightFog.hlsl"
           /* #ifdef _USE_DITHER
             #include "NGDither.hlsl"
            #endif*/

            #if  defined(CUBE_MAP) || defined(MAT_CAP)
            #include "NGCubeMap.hlsl"
            #endif
            ...
            struct Varyings
            {
             ...
             #endif
            #ifdef CUBE_MAP
                half3 refWS                     :TEXCOORD8;
            #endif
	     /*    #ifdef _USE_DITHER
		        float4 spos                      :TEXCOORD9;
            ...
            Varyings LitPassVertex(Attributes input)
            {
                ...
                output.uv.y += _TimeParameters.x * _YSpeed;
                output.uv.x += _TimeParameters.x * _XSpeed;
               ...
               //half3 objectSpaceViewDir = TransformWorldToObjectDir(viewDirWS);//half3(TransformWorldToObject(_WorldSpaceCameraPos.xyz) - input.positionOS.xyz);
               half3 objectSpaceViewDir =half3(TransformWorldToObject( GetCameraPositionWS()) - input.positionOS.xyz);
			   output.viewDirTS  = mul(rotation, objectSpaceViewDir).xyz;
#endif
            ...
            #endif
            #ifdef CUBE_MAP
                output.refWS = reflect(-viewDirWS, normalInput.normalWS);
            #endif
                output.positionCS = vertexInput.positionCS;
            //#ifdef _USE_DITHER
	           // output.spos = ComputeScreenPos( output.positionCS );
	           // output.spos.xy /= output.spos.w;
            //#endif

        

             #ifdef HEIGHT_FOG
                //output.uv.z = CalculateFogVS(output.positionWS);
                //half fogFactor = ComputeFogFactor(vertexInput.positionCS.z);
            ...
            half3 DirectStylizedBDRF(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS, half metallic)
            {
            #ifndef _SPECULARHIGHLIGHTS_OFF
                float3 halfDir = SafeNormalize(float3(lightDirectionWS) + float3(viewDirectionWS));
                half w = min(_SpecularLightOffset.w, 0.99);
                half NoH = saturate(dot(normalWS, halfDir));

                NoH = lerp(NoH, smoothstep(w,1,NoH), step(0.2176, metallic));
                half LoH = saturate(dot(lightDirectionWS, halfDir));
                LoH = lerp(LoH, smoothstep(w,1,LoH), step(0.2176, metallic));
                ...
                //specularTerm = smoothstep(_SpecularLightOffset.w, specularTerm, pow(specularTerm, 3));
            ...
            half3 LightingStylizedPhysicallyBased(BRDFData brdfData, half3 radiance, half3 lightColor, half3 lightDirectionWS, half lightAttenuation, half3 normalWS, half3 viewDirectionWS, half metallic)
            {
                return  DirectStylizedBDRF(brdfData, normalWS, normalize(lightDirectionWS + _SpecularLightOffset.xyz), viewDirectionWS, metallic) * radiance;
            }

            half3 LightingStylizedPhysicallyBased(BRDFData brdfData, half3 radiance, Light light, half3 normalWS, half3 viewDirectionWS, half metallic)
            {
                return LightingStylizedPhysicallyBased(brdfData, radiance, light.color, light.direction, light.distanceAttenuation * light.shadowAttenuation, normalWS, viewDirectionWS, metallic);
            }
            ...
            half3 CalculateRadiance(Light light, half3 normalWS)
            {
                ...
                half3 sColor = lerp(_ShadowColor.rgb,  _GlobalShadowColor.rgb,  _GlobalShadowColor.a);
                half3 ShadowColor = lerp(sColor, MedToneColor, smoothShadow );  
                //half3 ShadowColor = lerp( _GlobalShadowColor.rgb, MedToneColor, smoothShadow );  
            ...
            half smoothness, half occlusion, half3 emission, half alpha, half2 uv)    
            {
                ...
                Light mainLight = GetMainLight(inputData.shadowCoord, inputData.positionWS, shadowMask);//GetMainLight(inputData.shadowCoord);
                float3 radiance = CalculateRadiance(mainLight, inputData.normalWS);
                ...
                color += LightingStylizedPhysicallyBased(brdfData, radiance, mainLight, inputData.normalWS, inputData.viewDirectionWS, metallic);
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
               ...
               /* #ifdef _USE_DITHER
	                ditherClip(input.spos, _BaseColor.a);
                    if(_BaseColor.a < 1)
                    {
                        _ReceiveShadows = 0;
                        _ShadowThreshold = 0;
                ...
                #endif*/
                #ifdef HEIGHT_MAP
                float2 uvOffset = ParallaxOffset(input.uv, input.viewDirTS);
                input.uv += uvOffset;
                //input.uv = ReliefMapping(input.uv,input.viewDirWS);
                //input.uv = ParallaxMap(input.uv, input.viewDirWS, input.normalWS);
                #endif
                ...
                surfaceData.albedo = CubeMapReflect(surfaceData.albedo, input.refWS, input.uv, inputData.positionWS, surfaceData.specular.b);//     output.refWS = reflect�� - viewDirWS, normalWS);
                #elif MAT_CAP
                surfaceData.albedo = MatCapReflect(surfaceData.albedo, inputData.normalWS, input.uv, surfaceData.specular.b);
                #endif
            
                half4 color = UniversalFragmentStylizedPBR(inputData, surfaceData.albedo, surfaceData.metallic, surfaceData.specular, surfaceData.smoothness, surfaceData.occlusion, surfaceData.emission, surfaceData.alpha, input.uv.xy);
                //half4 color = UniversalFragmentPBR(inputData, surfaceData);
                 /*
                #ifdef CUBE_MAP
                color.rgb = CubeMapReflect(color.rgb, input.refWS);//     output.refWS = reflect�� - viewDirWS, normalWS);
                #elif MAT_CAP
                color.rgb = MatCapReflect(color.rgb, inputData.normalWS);
                #endif
                */
                ...
                #ifdef HEIGHT_FOG
                    color = DoHeightFogStuff(color,input.positionOS, input.positionWS);
                #endif
    ...
    CustomEditor "UnityEditor.Rendering.Universal.ShaderGUI.NGStylizedLitShader"
}
