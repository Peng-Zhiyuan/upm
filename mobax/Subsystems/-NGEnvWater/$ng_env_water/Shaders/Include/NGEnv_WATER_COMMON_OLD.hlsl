#ifndef WATER_COMMON_OLD_INCLUDED
#define WATER_COMMON_OLD_INCLUDED

	#include "../Include/NGEnv_WATER_SURFACEDATA_OLD.hlsl"
...
*/
	...
	half4 frag(Varyings IN) : SV_Target {
        ...
        float depthTest = exp2(-Distance * _DepthDistance);
// return depthTest;
		//VECTOR
		...
		//Color
		float4 Color = lerp(_DeepColor, _ShallowColor, depthTest);
		...
		//F
		float3 Fresnel = (_FresnelStrength * pow(1.0 - saturate(dot(normalize(normalWS), normalize(vDir))), _FresnelPower) * _FresnelColor.rgb);
		...
		//COLOR
		return half4(FinalColor2.rgb, Color.a * (1 - depthTest));
	}
#endif