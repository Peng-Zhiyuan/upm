...
namespace UnityEngine.Rendering.Universal
{
    ...
    public class PlanarReflections : MonoBehaviour
    {
        ...
        private void UpdateReflectionCamera(Camera realCamera)
        {
            ...
            Vector3 pos = realCamera.transform.position + Vector3.up * m_planeOffset;//Vector3.zero;
        ...
        private void PlanarReflectionTexture(Camera cam)
        {
            if (_reflectionTexture == null)
            {
                ...
                _blurTexture = RenderTexture.GetTemporary(res.x, res.y, 16,
                    GraphicsFormatUtility.GetGraphicsFormat(hdrFormat, true));
                _blurTexture.useMipMap = true;
                _blurTexture.autoGenerateMips = true;
        ...
        string mProfilerTag = "WaterRender";
        private void ExecutePlanarReflections(ScriptableRenderContext context, Camera camera)
        {
            ...
            if (camera.GetUniversalAdditionalCameraData().renderType == CameraRenderType.Overlay || camera.cameraType == CameraType.Reflection || camera.cameraType == CameraType.Preview)
                return;
            ...
            if (blurStrength > 0 && blurMat != null)
            {
                CommandBuffer cmd = CommandBufferPool.Get(mProfilerTag);
                blurMat.SetFloat("blurRangeX", blurStrength);
                blurMat.SetFloat("blurRangeY", blurStrength);
                cmd.Blit(_reflectionTexture, _blurTexture, blurMat);
                context.ExecuteCommandBuffer(cmd);
                CommandBufferPool.Release(cmd);
                Shader.SetGlobalTexture(_planarReflectionTextureId, _blurTexture);
            }
            else
            {
